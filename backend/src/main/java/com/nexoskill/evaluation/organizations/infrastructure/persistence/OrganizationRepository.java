...
public interface OrganizationRepository extends JpaRepository<OrganizationJpaEntity, Long> {
    ...
    List<OrganizationJpaEntity> findAllByOrganizationTypeAndStatusOrderByNameAsc(
            OrganizationType type, OrganizationStatus status);
    ...
    @Query("""
        select o
         ...
         where o.organizationType = :type
           ...
           and (o.expiresOn is null or o.expiresOn >= :today)
         order by lower(o.name), lower(o.code)
        """)
    List<OrganizationJpaEntity> findOperationalByTypeAndStatus(
