...
public record LicensePolicy(int contractedSeats, int includedReplacements, int additionalReplacements,
		int standardReleaseHours, int exhaustedReplacementReleaseDays, LocalDate cycleStartsOn, LocalDate cycleEndsOn) {

	public LicensePolicy {
		if (contractedSeats < 0)
			throw new IllegalArgumentException("Los asientos contratados no pueden ser negativos.");
		...
		if (standardReleaseHours < 1)
			throw new IllegalArgumentException("La liberación estándar debe ser mayor a cero.");
