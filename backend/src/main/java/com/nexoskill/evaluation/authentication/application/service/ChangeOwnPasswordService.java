...
public class ChangeOwnPasswordService {
	...
	public ChangeOwnPasswordService(UserRepository userRepository, PasswordHasher passwordHasher,
			PasswordPolicy passwordPolicy, PasswordHistoryPort passwordHistoryPort, UserSessionPort userSessionPort,
	...
	private void changeValidated(ChangePasswordCommand command) {
		...
		UserAccount user = userRepository.findById(command.userId())
				.orElseThrow(() -> new BusinessException("USER_NOT_FOUND", "La cuenta solicitada no existe."));

		if (!passwordHasher.matches(command.currentPassword(), user.getPasswordHash())) {
			throw new BusinessException("CURRENT_PASSWORD_INVALID", "La contraseña actual es incorrecta.");
		}
