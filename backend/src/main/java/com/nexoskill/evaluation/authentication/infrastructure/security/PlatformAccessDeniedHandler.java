...
public class PlatformAccessDeniedHandler implements AccessDeniedHandler {
	...
	private static final Set<String> READ_ONLY_MODULE_PREFIXES = Set.of("/api/v1/admin/catalogs",
			"/api/v1/admin/questions", "/api/v1/admin/forms", "/api/v1/admin/collections");
	...
	public void handle(HttpServletRequest request, HttpServletResponse response,
		...
		response.getWriter().write("{\"code\":\"" + denial.code() + "\",\"message\":\"" + denial.message() + "\"}");
	}

	/**
	 * Resuelve el mensaje funcional y registra el intento. También es utilizado por
	 * el manejador MVC porque las denegaciones de seguridad a nivel de método
	 * pueden ser resueltas por DispatcherServlet antes de regresar al filtro de
	 * seguridad.
	 */
	public Denial resolveAndAudit(HttpServletRequest request) {
		AuthenticatedUser user = authenticatedUser();
		boolean certificationOperationDenied = user != null && user.roles().contains("ADMINISTRATOR")
				&& isCertificationOperation(request.getRequestURI());
		boolean managerWriteDenied = user != null && user.roles().contains("MANAGER") && isWrite(request.getMethod())
				&& READ_ONLY_MODULE_PREFIXES.stream().anyMatch(request.getRequestURI()::startsWith);

		Denial denial = certificationOperationDenied ? new Denial("CERTIFICATION_OPERATION_FORBIDDEN",
				"La gestión operativa de certificaciones corresponde únicamente a Gestores y Supervisores de la organización.")
				: managerWriteDenied ? new Denial("ORGANIZATIONAL_MODULE_READ_ONLY",
	...
	private void auditDeniedAttempt(AuthenticatedUser user, HttpServletRequest request, String code) {
		try {
			...
			LOGGER.warn("No fue posible auditar la denegación de acceso a {} {}.", request.getMethod(),
