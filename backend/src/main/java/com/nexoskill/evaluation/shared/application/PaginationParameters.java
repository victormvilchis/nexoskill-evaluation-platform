 ...
 *
 * <p>
 * La validación vive en la capa de aplicación para evitar que los casos de uso
 * dependan de componentes REST.
 * </p>
 */
public final class PaginationParameters {
	...
	public static void validate(int page, int size) {
		...
		if (!ALLOWED_SIZES.contains(size) || size > MAX_SIZE) {
			throw new BusinessException("PAGINATION_SIZE_INVALID",
	...
	public static Pageable of(int page, int size, String requestedSort, String requestedDirection,
		...
		validate(page, size);
		String externalSort = requestedSort == null || requestedSort.isBlank() ? defaultSort : requestedSort.trim();
		...
		if (requestedDirection != null && !requestedDirection.isBlank()) {
			try {
				...
				throw new BusinessException("PAGINATION_DIRECTION_INVALID",
