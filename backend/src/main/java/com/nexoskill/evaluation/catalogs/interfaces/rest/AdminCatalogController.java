...
public class AdminCatalogController {
    ...
    public AdminCatalogController(CatalogAdministrationService service,
    ...
    @GetMapping("/{type}")
    @PreAuthorize("hasAuthority('CATALOG_VIEW')")
    public List<Item> items(@PathVariable String type,
            @RequestParam(defaultValue = "ACTIVE") String status,
            @RequestParam(required = false) String organizationPublicId,
            HttpServletRequest request) {
        return service.items(parse(type), status, organizationPublicId,
                tenantContextResolver.resolve(request));
    }

    @GetMapping("/{type}/{id}")
    @PreAuthorize("hasAuthority('CATALOG_VIEW')")
    public Item get(@PathVariable String type, @PathVariable String id,
            HttpServletRequest request) {
    ...
    public ResponseEntity<Item> create(@PathVariable String type,
    ...
    @PutMapping("/{type}/{id}")
    @PreAuthorize("hasAuthority('CATALOG_MANAGE')")
    public Item update(@PathVariable String type, @PathVariable String id,
            @Valid @RequestBody UpsertRequest body,
            @AuthenticationPrincipal AuthenticatedUser actor,
            HttpServletRequest request) {
    ...
    @PreAuthorize("hasAuthority('CATALOG_MANAGE')")
    public Item status(@PathVariable String type, @PathVariable String id,
            @PathVariable String action, @Valid @RequestBody StatusRequest body,
            @AuthenticationPrincipal AuthenticatedUser actor,
            HttpServletRequest request) {
        return service.changeStatus(parse(type), id,
                "activate".equals(action) ? "ACTIVE" : "INACTIVE",
                body.expectedVersion(), actor.internalId(), tenantContextResolver.resolve(request));
    }

    @GetMapping("/{type}/{id}/dependencies")
    @PreAuthorize("hasAuthority('CATALOG_VIEW')")
    public Dependencies dependencies(@PathVariable String type, @PathVariable String id,
            HttpServletRequest request) {
    ...
    public record UpsertRequest(
            ...
            String suggestedTechnologicalProfile,
