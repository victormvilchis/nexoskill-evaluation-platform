...
public class FormCreationTargetResolver {
	...
	public Target resolve(TenantContext tenant, String requestedScope, String organizationPublicId) {
		...
		if (!tenant.globalAdministrator()) {
			...
			return new Target(ContentScope.ORGANIZATION, tenant.organizationId(), tenant.organizationPublicId(),
					tenant.organizationCode(), tenant.organizationCode());
		}
		...
		if (scope == ContentScope.GLOBAL) {
			...
			return new Target(ContentScope.GLOBAL, global.getId(), global.getPublicId(), global.getCode(),
					global.getName());
		}
	...
	public Target fromExisting(ContentScope scope, Long organizationId) {
		...
		return new Target(scope, organization.getId(), organization.getPublicId(), organization.getCode(),
	...
	public List<FormModels.OrganizationOptionView> availableOrganizations(TenantContext tenant) {
		...
		return organizations
				.findAllByOrganizationTypeAndStatusOrderByNameAsc(OrganizationType.CUSTOMER, OrganizationStatus.ACTIVE)
				.stream().filter(organization -> organization.isOperational(today)).map(this::toOption).toList();
	}
	...
	public record Target(ContentScope scope, Long organizationId, String organizationPublicId, String organizationCode,
