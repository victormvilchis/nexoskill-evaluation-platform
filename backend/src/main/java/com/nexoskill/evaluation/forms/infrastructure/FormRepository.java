	...
	@Query(value = """
			...
			select count(form.id)
			 ...
			 where (
			   ...
			   and (:query is null or lower(form.title) like :query or lower(form.code) like :query)
			""")
	Page<FormJpaEntity> searchVisible(@Param("query") String query, @Param("status") String status,
			@Param("mode") String mode, @Param("globalContext") boolean globalContext,
			@Param("organizationId") Long organizationId, @Param("allowAllGlobal") boolean allowAllGlobal,
			@Param("organizationScope") ContentScope organizationScope, @Param("globalScope") ContentScope globalScope,
			@Param("contentType") GlobalContentType contentType, @Param("grantStatus") GrantStatus grantStatus,
