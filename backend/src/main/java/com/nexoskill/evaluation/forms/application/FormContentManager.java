...
public class FormContentManager {
	...
	public FormContentManager(FormSectionRepository sections, SpringDataQuestionRepository questions,
			SpringDataQuestionCategoryRepository categories, OrganizationRepository organizations,
	...
	public Snapshot load(Long formId, FormCreationTargetResolver.Target target) {
		...
		List<FormQuestionJpaEntity> questionLinks = storedSections.stream()
				...
				.sorted(Comparator.comparing(FormQuestionJpaEntity::getQuestionOrder)).toList();
		Map<Long, QuestionJpaEntity> questionById = questions
				.findAllById(questionLinks.stream().map(FormQuestionJpaEntity::getQuestionId).distinct().toList())
		...
		List<FormQuestionPoolJpaEntity> allPoolLinks = storedSections.stream()
				.flatMap(section -> section.getPools().stream()).toList();
		...
		List<FormQuestionPoolJpaEntity> poolLinks = allPoolLinks.stream()
				.sorted(Comparator.comparing(FormQuestionPoolJpaEntity::getPoolOrder)).toList();
		Map<Long, QuestionCategoryJpaEntity> categoryById = categories
				.findAllById(poolLinks.stream().map(FormQuestionPoolJpaEntity::getCategoryId).distinct().toList())
	...
	public ResolvedContent validate(FormContentMode mode, List<FormModels.QuestionItem> questionItems,
		...
		if (mode == FormContentMode.MANUAL) {
			if (safeQuestions.isEmpty()) {
				throw new BusinessException("FORM_QUESTIONS_REQUIRED",
						"Agrega al menos una pregunta activa al formulario.",
						Map.of("questions", "Agrega al menos una pregunta."));
			}
		...
		if (safePools.isEmpty()) {
			throw new BusinessException("FORM_POOLS_REQUIRED",
					"Configura al menos una categoría para el Pool aleatorio.",
					Map.of("pools", "Selecciona al menos una categoría."));
		}
		return new ResolvedContent(List.of(), resolvePools(safePools, target));
	}

	public FormModels.QuestionOptionPage questionOptions(FormCreationTargetResolver.Target target, String query,
		...
		String normalizedQuery = query == null || query.isBlank() ? null
				: "%" + query.trim().toLowerCase(Locale.ROOT) + "%";
		var result = questions.findActiveForForm(target.scope() == ContentScope.GLOBAL ? 1 : 0, target.organizationId(),
				normalizedQuery,
		...
		return new FormModels.QuestionOptionPage(result.getContent().stream().map(this::toQuestionOption).toList(),
	...
	private List<ResolvedQuestion> resolveQuestions(List<FormModels.QuestionItem> items,
		...
		for (FormModels.QuestionItem item : items.stream().filter(java.util.Objects::nonNull)
			...
			if (item == null || item.questionPublicId() == null || item.questionPublicId().isBlank()) {
				throw new BusinessException("FORM_QUESTION_INVALID",
						"Una de las preguntas seleccionadas no es válida.");
			}
		...
		if (byPublicId.size() != unique.size()) {
			throw new BusinessException("FORM_QUESTION_NOT_FOUND", "Una de las preguntas seleccionadas ya no existe.");
		}
	...
	private List<ResolvedPool> resolvePools(List<FormModels.PoolItem> items, FormCreationTargetResolver.Target target) {
		Set<String> seen = new HashSet<>();
		List<ResolvedPool> result = new ArrayList<>();
		for (FormModels.PoolItem item : items.stream().filter(java.util.Objects::nonNull)
				.sorted(Comparator.comparing(item -> item.order() == null ? Integer.MAX_VALUE : item.order()))
				.toList()) {
			...
			if (available < count) {
				throw new BusinessException("FORM_POOL_INSUFFICIENT_QUESTIONS", "La categoría " + category.getName()
						+ " solo tiene " + available + " preguntas activas disponibles para este alcance.");
			}
	...
	private boolean categoryCompatible(QuestionCategoryJpaEntity category, FormCreationTargetResolver.Target target) {
		if (target.scope() == ContentScope.GLOBAL) {
	...
	private int activeQuestionCount(QuestionCategoryJpaEntity category, FormCreationTargetResolver.Target target,
			String difficultyCode) {
		return (int) questions.findByStatusAndAnyCategoryIdIn(QuestionStatus.ACTIVE.name(), List.of(category.getId()))
				.stream().filter(question -> questionCompatible(question, target))
				.filter(question -> difficultyCode == null || (question.getDifficulty() != null
						&& difficultyCode.equalsIgnoreCase(question.getDifficulty().getCode())))
				.count();
	}

	private FormModels.QuestionView toQuestionView(QuestionJpaEntity question, int order, BigDecimal points,
			boolean required) {
		return new FormModels.QuestionView(question.getPublicId(), question.getStatement(),
				...
				question.getTechnology() == null ? null : question.getTechnology().getName(), question.getLevelCode(),
				categoryNames(question), question.getStatus().name(), order, points, required);
	}
	...
	private FormModels.QuestionOptionView toQuestionOption(QuestionJpaEntity question) {
		...
		return new FormModels.QuestionOptionView(question.getPublicId(), question.getStatement(),
				...
				question.getDifficulty() == null ? null : question.getDifficulty().getName(),
				question.getTechnology() == null ? null : question.getTechnology().getName(), question.getLevelCode(),
				categoryNames(question), question.getContentScope().name(),
