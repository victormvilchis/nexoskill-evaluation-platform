...
public class FormService {
	private static final Set<String> ALLOWED_STATUSES = Set.of("DRAFT", "ACTIVE", "DISABLED", "CLOSED", "ARCHIVED");
	...
	public FormService(FormRepository repository, TenantContextResolver tenantContextResolver,
			GlobalContentAccessPolicy accessPolicy, ContentSynchronizationService synchronization,
			FormCreationTargetResolver targetResolver, FormContentManager contentManager, HttpServletRequest request) {
	...
	public List<FormModels.FormSummary> list(String requestedStatus) {
		...
		return source.stream()
				...
				.sorted(Comparator.comparing(form -> form.title.toLowerCase(Locale.ROOT))).map(this::summary).toList();
	}
	...
	public Page<FormModels.FormSummary> list(String query, String requestedStatus, String requestedMode,
		...
		String normalizedMode = requestedMode == null || requestedMode.isBlank() ? null
		...
		var tenant = tenantContextResolver.resolve(request);
		return repository
				.searchVisible(normalizedQuery, normalizedStatus, normalizedMode, tenant.globalScope(),
						tenant.organizationId(), accessPolicy.allowsAllGlobal(tenant), ContentScope.ORGANIZATION,
						ContentScope.GLOBAL, GlobalContentType.FORM, GrantStatus.ACTIVE,
	...
	public FormModels.FormDetail create(FormModels.FormCommand command, Long actorUserId) {
		...
		var tenant = tenantContextResolver.resolve(request);
		FormCreationTargetResolver.Target target = targetResolver.resolve(tenant, command.contentScope(),
				command.organizationPublicId());
		FormContentMode contentMode = FormContentMode.parse(command.contentMode());
		FormContentManager.ResolvedContent content = contentManager.validate(contentMode, questions(command),
				pools(command), target, Set.of());
	...
	public FormModels.FormDetail update(String publicId, FormModels.FormCommand command, Long actorUserId) {
		...
		FormContentMode contentMode = FormContentMode.parse(command.contentMode());
		FormContentManager.ResolvedContent content = contentManager.validate(contentMode, questions(command),
				pools(command), target, current.questionPublicIds());
	...
	public FormModels.FormDetail clone(String publicId, FormModels.CloneCommand command, Long actorUserId) {
		...
		var tenant = tenantContextResolver.resolve(request);
		FormCreationTargetResolver.Target target = targetResolver.resolve(tenant, command.targetScope(),
				command.organizationPublicId());
		...
		List<FormModels.QuestionItem> questionItems = sourceContent.questions().stream()
				.map(item -> new FormModels.QuestionItem(item.questionPublicId(), item.order(), item.points(),
						item.required()))
				.toList();
		List<FormModels.PoolItem> poolItems = sourceContent.pools().stream()
				.map(item -> new FormModels.PoolItem(item.publicId(), "CATEGORY", item.categoryPublicId(),
						item.questionCount(), item.difficultyCode(), item.order()))
				.toList();
		FormContentManager.ResolvedContent validated = contentManager.validate(contentMode, questionItems, poolItems,
				target, Set.of());
	...
	@Transactional(readOnly = true)
	public FormModels.QuestionOptionPage questionOptions(String scope, String organizationPublicId, String query,
	...
	private FormModels.FormSummary summary(FormJpaEntity form) {
		FormCreationTargetResolver.Target owner = targetResolver.fromExisting(form.contentScope,
				form.ownerOrganizationId);
		FormContentManager.Counts counts = contentManager.counts(form.id);
		return new FormModels.FormSummary(form.publicId, form.code, form.title, form.status, form.modeCode,
				form.passingScore, form.contentScope.name(), owner.organizationPublicId(), owner.organizationName(),
				FormContentMode.parse(form.contentMode).name(), counts.questionCount(), counts.poolCount(),
				form.startsAt, form.endsAt, form.version);
	}
	...
	private FormModels.FormDetail detail(FormJpaEntity form, FormCreationTargetResolver.Target owner) {
		...
		return new FormModels.FormDetail(form.publicId, form.code, form.title, form.description, form.status,
				...
				owner.organizationName(), FormContentMode.parse(form.contentMode).name(), content.questions(),
				content.pools());
	}
	...
	private void validateHeader(FormModels.FormCommand command) {
		...
		if (!errors.isEmpty()) {
			throw new BusinessException("FORM_VALIDATION_ERROR", "Revisa la información obligatoria del formulario.",
					errors);
		}
	}
		...
		command.sections().stream()
				...
				.forEach(section -> {
					if (section.pools() != null)
						result.addAll(section.pools());
				});
		return result;
	}

	private String uniqueCode(String title) {
		String base = Normalizer.normalize(title, Normalizer.Form.NFD).replaceAll("\\p{M}", "").toUpperCase(Locale.ROOT)
