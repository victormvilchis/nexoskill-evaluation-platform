...
public class FormService {
	...
	public List<FormModels.FormSummary> list(String requestedStatus) {
		...
		return source.stream().sorted(Comparator.comparing(form -> form.title.toLowerCase(Locale.ROOT)))
	...
	private FormModels.FormDetail detail(FormJpaEntity form, List<FormModels.SectionView> sections) {
		return new FormModels.FormDetail(form.publicId, form.code, form.title, form.description, form.status,
				...
				form.acceptResponses == 1, form.startsAt, form.endsAt, form.durationMinutes, form.showResults == 1,
				form.showCorrectAnswers == 1, form.randomizeQuestions == 1, form.randomizeOptions == 1,
				form.showProgress == 1, form.hideQuestionNumbers == 1, form.allowSaveResume == 1,
				form.oneActiveAttempt == 1, form.thankYouMessage, form.version, sections);
	}
