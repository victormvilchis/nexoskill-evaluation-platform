...
public class StudentCertificationAttemptJpaEntity {
    ...
    @Column(name = "STUDENT_CERTIFICATION_ATTEMPT_ID") private Long id;
    @Column(name = "PUBLIC_ID", nullable = false, length = 36, unique = true) private String publicId;
    @Column(name = "STUDENT_CERT_REQUIREMENT_ID", nullable = false) private Long requirementId;
    ...
    @Column(name = "RESULT", length = 30) private String result;
    ...
    public static StudentCertificationAttemptJpaEntity create(Long requirementId, Long organizationId,
            int attemptNumber, LocalDate scheduledDate, LocalDate applicationDate,
            CertificationExamStatus examStatus, BigDecimal score, String result, String observations,
            Long actorId, Instant now) {
        if (attemptNumber < 1) throw new BusinessException("CERTIFICATION_ATTEMPT_INVALID", "El intento debe iniciar en 1.");
        ...
        String normalizedResult = result == null || result.isBlank()
                ? null : result.trim().toUpperCase(java.util.Locale.ROOT);
        if (normalizedResult != null && !java.util.Set.of(
                "PASSED", "FAILED", "ABSENT", "CANCELLED", "PENDING").contains(normalizedResult)) {
