...
/**
 * Consolida el estado vigente por área de certificación para evitar que varios ciclos históricos
 * inflen o contradigan los indicadores ejecutivos del colaborador.
 */
final class CertificationSummaryCalculator {
    ...
    static Metrics calculate(Applicability applicability, List<CycleView> cycles) {
        ...
        for (CertificationType type : CertificationType.values()) {
            if (!applies(applicability, type)) continue;
            applicableAreas++;

            List<CycleView> areaCycles = safeCycles.stream()
                    .filter(CycleView::active)
                    .filter(cycle -> cycle.type() == type)
                    .toList();
            AreaState state = evaluate(type, areaCycles);

            if (state.pendingInitial()) pending++;
            if (state.scheduled()) scheduled++;
            if (state.approved()) approved++;
            if (state.notApproved()) notApproved++;
            if (state.valid()) valid++;
            if (state.expiringSoon()) expiringSoon++;
            if (state.expired()) expired++;
            if (state.pendingRecertification()) pendingRecertifications++;
        }

        return new Metrics(applicableAreas, pending, scheduled, approved, notApproved,
                valid, expiringSoon, expired, pendingRecertifications);
    }

    private static AreaState evaluate(CertificationType type, List<CycleView> cycles) {
        boolean nonExpiring = nonExpiring(type);
        List<CycleView> approvedCycles = cycles.stream()
                .filter(CertificationSummaryCalculator::hasApprovedHistory)
                .toList();
        ...
        return new AreaState(pendingInitial, scheduled, approved, notApproved, valid,
                expiringSoon, expired, expired);
    }

    private static boolean hasApprovedHistory(CycleView cycle) {
        return Boolean.TRUE.equals(cycle.approved())
                || cycle.lastApprovedApplicationDate() != null
    ...
    private static boolean latestResultWasNotApproved(List<CycleView> cycles) {
        return cycles.stream()
                .filter(CertificationSummaryCalculator::hasResult)
    ...
    private static boolean hasResult(CycleView cycle) {
        return cycle.applicationDate() != null
                || cycle.trackingStatus() == CertificationTrackingStatus.APPROVED
    ...
    private static LocalDate resultDate(CycleView cycle) {
        if (cycle.applicationDate() != null) return cycle.applicationDate();
        if (cycle.lastApprovedApplicationDate() != null) return cycle.lastApprovedApplicationDate();
    ...
    private record AreaState(boolean pendingInitial, boolean scheduled, boolean approved,
            boolean notApproved, boolean valid, boolean expiringSoon, boolean expired,
            boolean pendingRecertification) {}
}
