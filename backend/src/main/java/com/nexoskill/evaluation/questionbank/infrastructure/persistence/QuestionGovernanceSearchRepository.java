...
public class QuestionGovernanceSearchRepository {
	...
	public SearchPage search(QuestionSearchFilter filter, TenantContext tenant, int page, int size) {
		...
		List<SearchRow> rows = jdbc.query(
				select + from + where + order + " OFFSET :offsetRows ROWS FETCH NEXT :pageSize ROWS ONLY ", params,
				this::mapRow);
	...
	public List<Integer> creationYears(TenantContext tenant) {
		...
		return jdbc.query("""
				SELECT DISTINCT EXTRACT(YEAR FROM q.CREATED_AT) CREATION_YEAR
				  FROM QUESTION q
				""" + where + " ORDER BY CREATION_YEAR DESC ", params, (rs, rowNum) -> rs.getInt("CREATION_YEAR"));
	}
	...
	private void addTenant(StringBuilder where, MapSqlParameterSource params, TenantContext tenant,
	...
	private void addFilters(StringBuilder where, MapSqlParameterSource params, QuestionSearchFilter filter,
			ContentScope scope) {
		...
		if (filter.status() == null)
			where.append(" AND q.STATUS <> 'DELETED' ");
		else {
		...
		optionalEquals(where, params, "owner_org.PUBLIC_ID", "ownerPublicId",
				blankToNull(filter.organizationPublicId()));
		...
		if (filter.clonedToGlobal() != null) {
			where.append(filter.clonedToGlobal() ? " AND q.SOURCE_ORGANIZATION_QUESTION_ID IS NOT NULL "
	...
	private void optionalEquals(StringBuilder where, MapSqlParameterSource params, String column, String parameter,
			String value) {
		if (value == null)
			return;
	...
	private void dateFrom(StringBuilder where, MapSqlParameterSource params, String column, String parameter,
			LocalDate value) {
		if (value == null)
			return;
		where.append(" AND ").append(column).append(" >= :").append(parameter).append(' ');
	...
	private void dateTo(StringBuilder where, MapSqlParameterSource params, String column, String parameter,
			LocalDate value) {
		if (value == null)
			return;
		where.append(" AND ").append(column).append(" < :").append(parameter).append(' ');
	...
	private SearchRow mapRow(ResultSet rs, int rowNum) throws SQLException {
		return new SearchRow(rs.getLong("QUESTION_ID"), rs.getString("OWNER_PUBLIC_ID"), rs.getString("OWNER_CODE"),
				rs.getString("OWNER_NAME"), rs.getString("TECHNOLOGY_PUBLIC_ID"), rs.getString("TECHNOLOGY_CODE"),
				rs.getString("TECHNOLOGY_NAME"), rs.getString("CREATOR_PUBLIC_ID"), rs.getString("CREATOR_NAME"),
				rs.getString("SOURCE_ORGANIZATION_PUBLIC_ID"), rs.getString("SOURCE_ORGANIZATION_NAME"),
				rs.getString("SOURCE_QUESTION_PUBLIC_ID"));
	}
	...
	public record SearchRow(Long questionId, String ownerOrganizationPublicId, String ownerOrganizationCode,
			String ownerOrganizationName, String technologyPublicId, String technologyCode, String technologyName,
			String creatorPublicId, String creatorName, String sourceOrganizationPublicId,
			String sourceOrganizationName, String sourceQuestionPublicId) {
		static SearchRow empty(Long questionId) {
			return new SearchRow(questionId, null, null, null, null, null, null, null, null, null, null, null);
		}
	}
}
