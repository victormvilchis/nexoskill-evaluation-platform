...
/**
 * Guarda la pregunta y, durante la creación GLOBAL, genera copias
 * organizacionales independientes dentro de la misma transacción. La
 * actualización conserva el flujo legado de disponibilidad únicamente cuando el
 * cliente lo solicita.
 */
@Service
public class QuestionSaveService {
	...
	private void distributeNewGlobalQuestion(QuestionDetail question, QuestionAvailabilityMode mode,
			List<String> organizationPublicIds, TenantContext tenant, Actor actor) {
		if (mode == null || mode == QuestionAvailabilityMode.NONE)
			return;
		...
		if (mode == QuestionAvailabilityMode.GLOBAL) {
			...
			if (organizationPublicIds != null) {
				organizationPublicIds.stream().filter(value -> value != null && !value.isBlank()).map(String::trim)
						.forEach(targets::add);
			}
			if (targets.isEmpty()) {
	...
	private void assertGlobalDistributionContext(QuestionDetail question, TenantContext tenant, Actor actor) {
		if (question == null || question.ownership() == null || question.ownership().scope() != ContentScope.GLOBAL) {
