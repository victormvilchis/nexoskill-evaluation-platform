...
public class AdminQuestionController {
	...
	public AdminQuestionController(QuestionServices.Search search, QuestionServices.Get get, QuestionSaveService save,
			QuestionServices.Duplicate duplicate, QuestionServices.CopyToOrganization copyToOrganization,
			...
			QuestionFilterOptionsService filterOptions, TenantContextResolver tenantContextResolver) {
		this.search = search;
		this.get = get;
		this.save = save;
	...
	public QuestionPage search(@RequestParam(required = false) String query,
			@RequestParam(defaultValue = "ACTIVE") String status, @RequestParam(required = false) String typeCode,
			@RequestParam(required = false) String categoryPublicId, @RequestParam(required = false) String scope,
			@RequestParam(required = false) String organizationPublicId,
			@RequestParam(required = false) String technologyPublicId,
			@RequestParam(required = false) String difficultyCode, @RequestParam(required = false) String levelCode,
			@RequestParam(required = false) String creatorPublicId, @RequestParam(required = false) Integer createdYear,
			...
			@RequestParam(required = false) Boolean clonedToGlobal, @RequestParam(required = false) Boolean inUse,
		...
		if (createdYear != null) {
			...
			if (createdYear < 2000 || createdYear > currentYear + 1) {
				throw new com.nexoskill.evaluation.shared.domain.BusinessException("QUESTION_CREATION_YEAR_INVALID",
						"El año de creación indicado no es válido.");
			}
		...
		return search.execute(query, status, typeCode, categoryPublicId, scope, organizationPublicId,
				technologyPublicId, difficultyCode, levelCode, creatorPublicId, effectiveCreatedFrom,
				effectiveCreatedTo, updatedFrom, updatedTo, clonedToGlobal, inUse, page, size);
	}
	...
	public ResponseEntity<QuestionDetail> create(@Valid @RequestBody QuestionRequests.Create body,
		...
		var question = save.create(
				new CreateQuestionCommand(body.typeCode(), body.difficultyCode(), body.technologyPublicId(),
						body.levelCode(), body.categoryPublicIds(), body.tags(), body.statement(), body.explanation(),
						body.promptMediaPublicId(), body.codeContent(), settings(body.answerSettings()),
						options(body.options()), body.contentScope(), body.organizationPublicId(), actor.internalId()),
				body.availabilityMode(), body.availabilityOrganizationPublicIds(),
	...
	public QuestionDetail update(@PathVariable String id, @Valid @RequestBody QuestionRequests.Update body,
			@AuthenticationPrincipal AuthenticatedUser actor, HttpServletRequest request) {
		return save.update(
				new UpdateQuestionCommand(id, body.typeCode(), body.difficultyCode(), body.technologyPublicId(),
						body.levelCode(), body.categoryPublicIds(), body.tags(), body.statement(), body.explanation(),
						body.promptMediaPublicId(), body.codeContent(), settings(body.answerSettings()),
						options(body.options()), body.expectedEntityVersion(), actor.internalId()),
				body.availabilityMode(), body.availabilityOrganizationPublicIds(),
				tenantContextResolver.resolve(request), new QuestionSaveService.Actor(actor.internalId(),
						ClientRequestInfo.ipAddress(request), ClientRequestInfo.userAgent(request)));
	}
	...
	public com.nexoskill.evaluation.questionbank.application.service.QuestionGlobalCloneService.CloneResult cloneToGlobal(
			@PathVariable String id, @RequestBody(required = false) QuestionRequests.CloneToGlobal body,
			@AuthenticationPrincipal AuthenticatedUser actor) {
