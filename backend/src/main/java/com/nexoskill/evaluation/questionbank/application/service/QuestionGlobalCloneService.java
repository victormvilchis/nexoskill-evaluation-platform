...
public class QuestionGlobalCloneService {
	...
	@Autowired
	public QuestionGlobalCloneService(SpringDataQuestionRepository questions, QuestionTagStore tagStore,
			GlobalContentPromotionRepository promotions, ContentPromotionService promotionService,
			TenantContextResolver tenantContextResolver, HttpServletRequest request, AuditLogPort audit, Clock clock) {
	...
	public QuestionGlobalCloneService(SpringDataQuestionRepository questions,
			GlobalContentPromotionRepository promotions, ContentPromotionService promotionService,
			TenantContextResolver tenantContextResolver, HttpServletRequest request, AuditLogPort audit, Clock clock) {
		this(questions, null, promotions, promotionService, tenantContextResolver, request, audit, clock);
	}

	@Transactional(readOnly = true)
	public com.nexoskill.evaluation.globalcontent.application.model.GlobalContentModels.PromotionPreview preview(
	...
	@Transactional
	public CloneResult cloneToGlobal(String questionPublicId, boolean includeDependencies, String notes,
			Long actorUserId) {
		ensureGlobalAdministrator();
		String normalized = PublicIdNormalizer.requiredUuid(questionPublicId, "QUESTION_ID_INVALID",
				"La pregunta indicada no es válida.");
		var source = questions.findByPublicId(normalized)
				.orElseThrow(() -> new BusinessException("QUESTION_NOT_FOUND", "La pregunta solicitada no existe."));
		...
		if (promotions.findByContentTypeAndSourceContentIdAndSourceVersion(GlobalContentType.QUESTION, source.getId(),
				source.getVersion()).isPresent()) {
			throw new BusinessException("QUESTION_GLOBAL_CLONE_EXISTS",
					"Ya existe una pregunta global relacionada con este contenido.");
		}
		var promotion = promotionService.promote(new PromoteCommand(GlobalContentType.QUESTION, source.getPublicId(),
				includeDependencies, DuplicateResolution.CREATE_DISTINCT, null, notes), actorUserId);
		...
		global.markClonedFromOrganization(source.getOwnerOrganizationId(), source.getId(), source.getVersion(),
				actorUserId, clock.instant());
		questions.saveAndFlush(global);
		if (tagStore != null) {
			tagStore.copy(source.getId(), global.getId(), ContentScope.GLOBAL, global.getOwnerOrganizationId(),
					actorUserId, clock.instant());
		}
		audit.record(actorUserId, "QUESTION_CLONED_TO_GLOBAL", "QUESTION_BANK",
				"Se clonó una pregunta organizacional al Banco de Preguntas Global.", null, null,
				Map.of("sourceQuestionPublicId", source.getPublicId(), "globalQuestionPublicId", global.getPublicId(),
						"sourceOrganizationId", source.getOwnerOrganizationId()),
				clock.instant());
	...
	public record CloneResult(String promotionPublicId, String sourceQuestionPublicId, String globalQuestionPublicId) {
	}
}
