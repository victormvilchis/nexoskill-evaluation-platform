...
public class OracleQuestionCollectionAdapter implements QuestionCollectionPort {
	...
	@Override
	public CollectionDetail create(CollectionCommands.Create command) {
		String name = name(command.name());
		String normalized = normalize(name);
		if (collections.existsByNormalizedName(normalized))
			throw error("COLLECTION_ALREADY_EXISTS", "Ya existe una colección con ese nombre.");
	...
	@Override
	public CollectionDetail update(CollectionCommands.Update command) {
		String id = PublicIdNormalizer.requiredUuid(command.publicId(), "COLLECTION_ID_INVALID",
				"La colección indicada no es válida.");
		var entity = collections.findForUpdate(id)
				.orElseThrow(() -> error("COLLECTION_NOT_FOUND", "La colección solicitada no existe."));
		if (entity.getVersion() != command.expectedEntityVersion())
			throw error("COLLECTION_CONCURRENT_MODIFICATION", "La colección fue modificada por otra persona.");
		String name = name(command.name());
		String normalized = normalize(name);
		if (collections.existsByNormalizedNameAndPublicIdNot(normalized, id))
			throw error("COLLECTION_ALREADY_EXISTS", "Ya existe una colección con ese nombre.");
		...
		entity.apply(name, normalized, nullable(command.description()),
				resolveCategories(command.categoryPublicIds(), retained), resolveQuestions(command.questionPublicIds()),
	...
	public CollectionDetail get(String id) {
		String publicId = PublicIdNormalizer.requiredUuid(id, "COLLECTION_ID_INVALID",
				"La colección indicada no es válida.");
		return detail(collections.findByPublicId(publicId)
				.orElseThrow(() -> error("COLLECTION_NOT_FOUND", "La colección solicitada no existe.")));
	}

	@Override
	public CollectionPage search(String query, String status, int page, int size) {
	...
	@Override
	public CollectionDetail changeStatus(CollectionCommands.ChangeStatus command) {
		String id = PublicIdNormalizer.requiredUuid(command.publicId(), "COLLECTION_ID_INVALID",
				"La colección indicada no es válida.");
		var entity = collections.findForUpdate(id)
				.orElseThrow(() -> error("COLLECTION_NOT_FOUND", "La colección solicitada no existe."));
		if (entity.getVersion() != command.expectedEntityVersion())
			throw error("COLLECTION_CONCURRENT_MODIFICATION",
	...
	private Set<QuestionCategoryJpaEntity> resolveCategories(List<String> ids, Set<String> existing) {
		Set<String> canonical = ids.stream().map(
				value -> PublicIdNormalizer.requiredUuid(value, "CATEGORY_ID_INVALID", "Una categoría no es válida."))
				.collect(Collectors.toCollection(LinkedHashSet::new));
		var found = categories.findAllByPublicIdIn(canonical);
		if (found.size() != canonical.size())
			throw error("CATEGORY_NOT_FOUND", "Una categoría seleccionada no existe.");
	...
	private Set<QuestionJpaEntity> resolveQuestions(List<String> ids) {
		Set<String> canonical = ids.stream().map(
				value -> PublicIdNormalizer.requiredUuid(value, "QUESTION_ID_INVALID", "Una pregunta no es válida."))
				.collect(Collectors.toCollection(LinkedHashSet::new));
		var found = questions.findAllByPublicIdIn(canonical);
		if (found.size() != canonical.size())
			throw error("QUESTION_NOT_FOUND", "Una pregunta seleccionada no existe.");
		for (var question : found) {
			if (question.getStatus() == QuestionStatus.DELETED)
				throw error("QUESTION_DELETED", "No se pueden agregar preguntas eliminadas.");
			if (question.getStatus() != QuestionStatus.ACTIVE)
				throw error("QUESTION_ARCHIVED", "No se pueden agregar preguntas archivadas.");
		}
		return new LinkedHashSet<>(found);
	}
	...
	private List<QuestionJpaEntity> effective(QuestionCollectionJpaEntity entity) {
		...
		if (!categoryIds.isEmpty())
			for (var question : questions.findByStatusAndAnyCategoryIdIn(QuestionStatus.ACTIVE.name(), categoryIds))
				result.put(question.getId(), question);
	...
	private CollectionSummary summary(QuestionCollectionJpaEntity entity) {
		return new CollectionSummary(entity.getPublicId(), entity.getName(), entity.getDescription(),
				entity.getStatus(), entity.getCategories().size(), visibleExplicit(entity).size(),
				effective(entity).size(), entity.getVersion(), entity.getCreatedAt(), entity.getUpdatedAt());
	}
	...
	private CollectionDetail detail(QuestionCollectionJpaEntity entity) {
		return new CollectionDetail(entity.getPublicId(), entity.getName(), entity.getDescription(), entity.getStatus(),
				entity.getCategories().stream()
						.map(category -> new QuestionCategoryRef(category.getPublicId(), category.getCode(),
								category.getName(), category.getStatus()))
						.sorted(Comparator.comparing(QuestionCategoryRef::name)).toList(),
				visibleExplicit(entity).stream().map(this::questionSummary).toList(),
				effective(entity).stream().map(this::questionSummary).toList(), entity.getVersion(),
	...
	private QuestionSummary questionSummary(QuestionJpaEntity question) {
		return new QuestionSummary(question.getPublicId(), question.getStatement(), question.getType().getCode(),
				...
				question.getDifficulty() == null ? null : question.getDifficulty().getName(), question.getLevelCode(),
				question.getTechnology() == null ? null
						: new QuestionTechnologySummary(question.getTechnology().getPublicId(),
								question.getTechnology().getCode(), question.getTechnology().getName(),
								question.getTechnology().getStatus().name(),
								question.getTechnology().getDisplayOrder()),
				...
				question.getCodeContent() != null && !question.getCodeContent().isBlank(), false, null,
				java.util.List.of(), java.util.List.of(), question.getVersion(), question.getCreatedAt(),
				question.getUpdatedAt());
	}

	private String name(String value) {
		if (value == null || value.isBlank())
			throw error("COLLECTION_NAME_REQUIRED", "El nombre es obligatorio.");
