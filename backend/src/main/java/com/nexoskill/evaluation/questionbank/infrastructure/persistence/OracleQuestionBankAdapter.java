...
public class OracleQuestionBankAdapter implements QuestionBankPort {
	...
	public OracleQuestionBankAdapter(SpringDataQuestionRepository questions, SpringDataQuestionTypeRepository types,
			SpringDataQuestionDifficultyRepository difficulties, SpringDataQuestionCategoryRepository categories,
			SpringDataQuestionMediaRepository media, QuestionUsageChecker usage, ObjectMapper json, Clock clock,
	...
	public QuestionDetail create(CreateQuestionCommand command) {
		...
		var entity = QuestionJpaEntity.create(UUID.randomUUID().toString(), selection.type(), internalDifficulty(),
	...
	public QuestionDetail update(UpdateQuestionCommand command) {
		...
		entity.clearOptions();
		questions.flush();
		entity.apply(selection.type(), internalDifficulty(), selection.categories(), trim(command.statement()),
	...
	@Override
	public QuestionPage search(String query, QuestionStatus status, String type, String category, int page, int size) {
		...
		Page<QuestionJpaEntity> result = normalizedQuery == null
				? questions.searchWithoutText(status == null ? null : status.name(), normalizedType, normalizedCategory,
						pageable)
				: questions.searchWithText(normalizedQuery, status == null ? null : status.name(), normalizedType,
						normalizedCategory, pageable);
		MembershipIndex membershipIndex = memberships(
				result.getContent().stream().map(QuestionJpaEntity::getId).toList());
	...
	public QuestionDetail duplicate(String id, Long actor) {
		...
		var entity = QuestionJpaEntity.create(UUID.randomUUID().toString(), source.getType(), internalDifficulty(),
				new LinkedHashSet<>(source.getCategories()), source.getStatement() + " (copia)",
				...
				source.isManualReview(), null, null, null, source.getResponseMaxLength(), actor, clock.instant());
		for (var option : source.getOptions()) {
			entity.addOption(QuestionOptionJpaEntity.create(entity, UUID.randomUUID().toString(),
					...
					option.getMatchMedia(), option.isCorrect(), option.getFeedback(), clock.instant()));
		}
		QuestionJpaEntity saved = questions.saveAndFlush(entity);
		return toDetail(saved, memberships(List.of(saved.getId())));
	}
	...
	private void addOptions(QuestionJpaEntity entity, List<QuestionOptionCommand> commands) {
		...
		for (var command : commands) {
			entity.addOption(QuestionOptionJpaEntity.create(entity, UUID.randomUUID().toString(), order++,
					nullable(command.text()), optionalMedia(command.mediaPublicId()), nullable(command.matchText()),
					optionalMedia(command.matchMediaPublicId()), command.correct(), nullable(command.feedback()),
					clock.instant()));
		}
	}
	...
	private QuestionDetail toDetail(QuestionJpaEntity entity, MembershipIndex membershipIndex) {
		return new QuestionDetail(entity.getPublicId(), entity.getStatement(), entity.getExplanation(),
				...
				entity.getOptions().stream()
						...
						.toList(),
				membershipIndex.forms(entity.getId()), membershipIndex.collections(entity.getId()),
	...
	private QuestionSummary toSummary(QuestionJpaEntity entity, MembershipIndex membershipIndex) {
		return new QuestionSummary(entity.getPublicId(), entity.getStatement(), entity.getType().getCode(),
				...
				membershipIndex.forms(entity.getId()), membershipIndex.collections(entity.getId()), entity.getVersion(),
				entity.getCreatedAt(), entity.getUpdatedAt());
	}
