...
public final class QuestionServices {
	...
	public static class Search {
		...
		@Transactional(readOnly = true)
		public QuestionPage execute(String query, String statusValue, String type, String category, String scopeValue,
				String organizationPublicId, String technologyPublicId, String difficultyCode, String levelCode,
				String creatorPublicId, java.time.LocalDate createdFrom, java.time.LocalDate createdTo,
				java.time.LocalDate updatedFrom, java.time.LocalDate updatedTo, Boolean clonedToGlobal, Boolean inUse,
				int page, int size) {
			...
			QuestionSearchFilter filter = new QuestionSearchFilter(query, status, type, category, scope,
					organizationPublicId, technologyPublicId, difficultyCode, levelCode, creatorPublicId, createdFrom,
					createdTo, updatedFrom, updatedTo, clonedToGlobal, inUse);
			return port.search(filter, Math.max(0, page), Math.min(Math.max(size, 1), 100));
		}

		public QuestionPage execute(String query, String statusValue, String type, String category, int page,
				int size) {
			return execute(query, statusValue, type, category, null, null, null, null, null, null, null, null, null,
	...
	public static class Duplicate {
		...
		public Duplicate(QuestionBankPort port, TenantContextResolver tenantContextResolver,
		...
		@Transactional
		public QuestionDetail execute(String id, String requestedTargetScope, String requestedOrganizationPublicId,
				Long actor) {
			...
			if (source.status() == QuestionStatus.DELETED) {
				throw new BusinessException("QUESTION_DELETED", "No se puede duplicar una pregunta eliminada.");
			}
			if (actor == null) {
		...
		private static void assertCanDuplicate(QuestionDetail source,
			...
			if (tenant != null && tenant.globalAdministrator())
				return;
			if (tenant == null || !tenant.hasOrganization() || source.ownership().scope() != ContentScope.ORGANIZATION
					|| !Objects.equals(source.ownership().organizationPublicId(), tenant.organizationPublicId())) {
	...
	public static class CopyToOrganization {
		...
		public CopyToOrganization(QuestionBankPort port) {
			this.port = port;
		}

		@Transactional
		public QuestionDetail execute(String id, Long actor) {
			return port.copyToOrganization(id, actor);
		}
	}
