...
/**
 * Verifica usos operativos y retira relaciones directas cuando una pregunta se
 * elimina lógicamente. Los pools por categoría se conservan porque no almacenan
 * una referencia directa a la pregunta y solo consumen preguntas activas.
 */
@Component
public class OracleQuestionUsageChecker implements QuestionUsageChecker {
	...
	public DetachmentResult detachFromForms(Long questionId) {
		if (questionId == null)
			return new DetachmentResult(0, 0);
		Map<String, Object> params = Map.of("questionId", questionId);
		int fixedRelations = jdbc.update("DELETE FROM FORM_QUESTION WHERE QUESTION_ID = :questionId", params);
		int collectionRelations = jdbc
				.update("DELETE FROM COLLECTION_QUESTION_RELATION WHERE QUESTION_ID = :questionId", params);
