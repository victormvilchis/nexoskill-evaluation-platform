...
public class OracleQuestionCatalogAdapter implements QuestionCatalogPort {
	...
	public OracleQuestionCatalogAdapter(SpringDataQuestionTypeRepository types,
			SpringDataQuestionDifficultyRepository ignoredDifficulties, SpringDataQuestionCategoryRepository categories,
			SpringDataQuestionRepository questions, QuestionCategoryStatusHistoryRepository historyRepository,
	...
	public QuestionCatalogs activeCatalogs(TenantContext tenant) {
		return new QuestionCatalogs(
				...
				categories.findVisible(tenant.organizationId(), CatalogStatus.ACTIVE).stream().map(this::summary)
						.toList());
	}
	...
	private ScopeTarget scope(TenantContext tenant) {
		if (tenant.globalScope())
			return new ScopeTarget(ContentScope.GLOBAL, null);
