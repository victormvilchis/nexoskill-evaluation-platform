...
public class OracleQuestionCatalogAdapter implements QuestionCatalogPort {
	...
	public OracleQuestionCatalogAdapter(SpringDataQuestionTypeRepository types,
			...
			SpringDataQuestionCategoryRepository categories, SpringDataQuestionRepository questions,
			QuestionCategoryStatusHistoryRepository historyRepository, OrganizationRepository organizationRepository,
			GlobalContentAccessPolicy accessPolicy, QuestionCreationTargetResolver creationTargetResolver,
			ContentSynchronizationService synchronization, Clock clock) {
		this.types = types;
	...
	public QuestionCatalogs activeCatalogs(TenantContext tenant) {
		return new QuestionCatalogs(
				...
				technologies
						.findByStatusOrderByDisplayOrderAscNameAsc(
								com.nexoskill.evaluation.questionbank.domain.model.QuestionTechnologyStatus.ACTIVE)
						.stream()
						.map(value -> new QuestionTechnologySummary(value.getPublicId(), value.getCode(),
								value.getName(), value.getStatus().name(), value.getDisplayOrder()))
						.toList(),
	...
	public List<QuestionCategorySummary> questionOptions(TenantContext tenant, String questionPublicId,
		...
		if (target.scope() == ContentScope.GLOBAL) {
			...
			categories.findAllByContentScopeAndOwnerOrganizationIdOrderByNameAsc(ContentScope.ORGANIZATION,
					target.organizationId()).forEach(value -> candidates.put(value.getId(), value));
			target.retainedCategories().forEach(value -> candidates.putIfAbsent(value.getId(), value));
		}

		Set<Long> retained = target.retainedCategories().stream().map(QuestionCategoryJpaEntity::getId)
	...
	public List<QuestionTechnologySummary> technologyOptions(TenantContext tenant, String questionPublicId,
		...
		if (target.scope() == ContentScope.GLOBAL) {
			...
			technologies
					.findAllByContentScopeAndOwnerOrganizationIdOrderByDisplayOrderAscNameAsc(ContentScope.ORGANIZATION,
		...
		return candidates.values().stream().filter(value -> value
				.getStatus() == com.nexoskill.evaluation.questionbank.domain.model.QuestionTechnologyStatus.ACTIVE
	...
	private QuestionOptionTarget optionTarget(TenantContext tenant, String questionPublicId, String targetScopeValue,
			String organizationPublicId) {
		if (questionPublicId == null || questionPublicId.isBlank()) {
			QuestionCreationTargetResolver.Target target = creationTargetResolver.resolve(tenant, targetScopeValue,
	...
	public QuestionCategorySummary create(CategoryCommands.Create command) {
		...
		try {
			entity = QuestionCategoryJpaEntity.create(UUID.randomUUID().toString(), code, name,
					nullable(command.description()), target.scope(), target.organizationId(), command.actorUserId(),
					now);
	...
	public QuestionCategorySummary changeStatus(CategoryCommands.ChangeStatus command) {
		...
		if (command.status() == CatalogStatus.ACTIVE) {
			...
			if (previous != CatalogStatus.ACTIVE) {
				throw error("CATEGORY_STATUS_TRANSITION_INVALID", "Solamente una categoría activa puede inactivarse.");
			}
	...
	public QuestionCategorySummary softDelete(CategoryCommands.Delete command) {
		...
		if (entity.getStatus() != CatalogStatus.INACTIVE) {
			throw error("CATEGORY_MUST_BE_INACTIVE", "La categoría debe estar inactiva antes de eliminarse.");
		}
	...
	public QuestionCategoryDependencies dependencies(String publicId, TenantContext tenant) {
		...
		String message = count > 0
				? "No es posible eliminar la categoría porque está siendo utilizada por preguntas existentes."
				: entity.getStatus() != CatalogStatus.INACTIVE ? "La categoría debe estar inactiva antes de eliminarse."
	...
	private QuestionCategoryJpaEntity readable(String publicId, TenantContext tenant) {
		...
		accessPolicy.assertReadable(GlobalContentType.CATEGORY, entity.getId(), entity.getContentScope(),
				entity.getOwnerOrganizationId(), tenant, "CATEGORY_NOT_FOUND", "La categoría solicitada no existe.");
		return entity;
	}
	...
	private ScopeTarget scope(TenantContext tenant) {
		if (tenant.globalScope())
			return new ScopeTarget(ContentScope.GLOBAL, tenant.organizationId());
		if (!tenant.hasOrganization()) {
			throw error("TENANT_NOT_RESOLVED",
	...
	private void ensureUnique(String code, String name, ContentScope scope, Long organizationId, String ignore) {
		for (QuestionCategoryJpaEntity entity : categories.findWithinScope(scope, organizationId)) {
			if (ignore != null && entity.getPublicId().equals(ignore))
				continue;
	...
	private record QuestionOptionTarget(ContentScope scope, Long organizationId,
			List<QuestionCategoryJpaEntity> retainedCategories, QuestionTechnologyJpaEntity retainedTechnology) {
	}
