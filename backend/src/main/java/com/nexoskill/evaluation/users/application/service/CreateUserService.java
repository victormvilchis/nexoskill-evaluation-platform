...
public class CreateUserService {
	...
	public CreateUserResult create(CreateUserCommand command) {
		...
		var created = userManagementPort
				.create(new UserManagementPort.NewUserData(UUID.randomUUID().toString(), command.email().trim(),
						normalizedEmail, passwordHasher.encode(temporaryPassword), command.firstName().trim(),
						command.lastName().trim(),
						UserManagementSupport.displayName(command.firstName(), command.lastName(),
								command.displayName()),
						roleCode, normalizeOrganization(command.organizationPublicId()), initialStatus,
						command.startsAt(), command.expiresAt(),
						now.plus(properties.getSecurity().getTemporaryPasswordDuration()), command.actorUserId(), now));
		...
		auditLogPort.record(command.actorUserId(), "USER_CREATED", "USER_MANAGEMENT",
				...
				command.userAgent(), Map.of("targetUserPublicId", created.publicId(), "targetEmail", created.email(),
						"role", roleCode, "initialStatus", initialStatus.name()),
				now);
		auditLogPort.record(command.actorUserId(), "TEMPORARY_PASSWORD_GENERATED", "USER_MANAGEMENT",
				"Se generó una contraseña temporal para un usuario interno.", command.ipAddress(), command.userAgent(),
				Map.of("targetUserPublicId", created.publicId()), now);
