...
public class UserController {
    ...
    public UserController(
            GetOwnProfileService getOwnProfileService,
    ...
    public ResponseEntity<OwnProfileResponse> profile(
            @AuthenticationPrincipal AuthenticatedUser principal) {
        return ResponseEntity.ok(new OwnProfileResponse(
                getOwnProfileService.get(principal.publicId())
        ));
    }

    @PutMapping("/me/profile")
    @PreAuthorize("hasAuthority('PROFILE_UPDATE')")
    public ResponseEntity<OwnProfileResponse> updateProfile(
            @AuthenticationPrincipal AuthenticatedUser principal,
            @Valid @RequestBody UpdateOwnProfileRequest body,
            HttpServletRequest request) {
        return ResponseEntity.ok(new OwnProfileResponse(
                updateOwnProfileService.update(new UpdateOwnProfileCommand(
