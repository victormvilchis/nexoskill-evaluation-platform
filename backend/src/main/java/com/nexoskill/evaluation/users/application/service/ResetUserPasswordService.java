...
public class ResetUserPasswordService {
	...
	public AdminUserSummary reset(ResetUserPasswordCommand command) {
		...
		AdminUserSummary updated = userManagementPort.updatePassword(command.publicId(),
		...
		data.put("temporaryPasswordExpiresAt",
		...
		auditLogPort.record(command.actorUserId(), "USER_PASSWORD_RESET", "USER_MANAGEMENT",
				"Se restableció la contraseña temporal de un usuario.", command.ipAddress(), command.userAgent(), data,
				now);
		return updated;
	}
