...
public class UpdateUserProfileService {
	...
	public UpdateUserProfileService(UserManagementPort userManagementPort, AuditLogPort auditLogPort, Clock clock) {
	...
	public AdminUserSummary update(UpdateUserProfileCommand command) {
		...
		if (userManagementPort.existsByNormalizedEmailExcluding(normalizedEmail, command.publicId())) {
			throw new BusinessException("USER_EMAIL_EXISTS", "Ya existe otro usuario registrado con ese correo.");
		}

		AdminUserSummary updated = userManagementPort.updateProfile(command.publicId(), command.email().trim(),
				normalizedEmail, command.firstName().trim(), command.lastName().trim(),
				UserManagementSupport.displayName(command.firstName(), command.lastName(), command.displayName()));

		auditLogPort.record(command.actorUserId(), "USER_UPDATED", "USER_MANAGEMENT",
				"Se actualizaron los datos generales de un usuario.", command.ipAddress(), command.userAgent(),
