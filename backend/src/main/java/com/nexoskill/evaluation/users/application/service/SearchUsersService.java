...
public class SearchUsersService {
	...
	public AdminUserPage search(String query, String status, int page, int size) {

		if (page < 0 || size < 1 || size > 100) {
			throw new BusinessException("PAGINATION_INVALID", "La paginación solicitada no es válida.");
		}

		UserStatus parsedStatus = parseStatus(status);
		String normalizedQuery = query == null || query.isBlank() ? null : query.trim().toLowerCase();

		return userManagementPort.search(normalizedQuery, parsedStatus, page, size);
	}
	...
	private UserStatus parseStatus(String status) {
		...
		try {
			...
			throw new BusinessException("USER_STATUS_INVALID", "El estado de usuario indicado no es válido.");
		}
	}
}
