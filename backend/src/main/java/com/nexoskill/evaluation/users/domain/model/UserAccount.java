...
public class UserAccount {
	...
	public UserAccount(Long id, String publicId, String email, String normalizedEmail, String passwordHash,
			String firstName, String lastName, String displayName, UserStatus status, int failedLoginAttempts,
			Instant lockedUntil, Instant lastLoginAt, boolean passwordChangeRequired, Instant passwordChangedAt,
			Instant temporaryPasswordExpiresAt, Set<RoleGrant> roles, UserAccess access) {
	...
	public Instant capSessionExpirationForPassword(Instant requestedExpiration) {
		...
		return temporaryPasswordExpiresAt.isBefore(requestedExpiration) ? temporaryPasswordExpiresAt
				: requestedExpiration;
	}
	...
	public void registerFailedLogin(int maximumAttempts, Instant now, Duration lockDuration) {
	...
	public Set<String> permissions() {
		return roles.stream().flatMap(role -> role.permissions().stream())
