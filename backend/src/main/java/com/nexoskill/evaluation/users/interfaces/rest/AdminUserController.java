...
public class AdminUserController {
	...
	public AdminUserController(SearchUsersService searchUsersService, CreateUserService createUserService,
			...
			ResetUserPasswordService resetUserPasswordService, InternalUserStatusHistoryService statusHistoryService,
			InternalUserSessionService sessionService) {
	...
	public ResponseEntity<CreateUserResponse> create(@Valid @RequestBody CreateUserRequest body,
		...
		CreateUserResult result = createUserService.create(new CreateUserCommand(body.email(), body.firstName(),
				body.lastName(), body.displayName(), body.roleCode(), body.organizationPublicId(), body.initialStatus(),
				body.startsAt(), body.expiresAt(), actor.internalId(), ClientRequestInfo.ipAddress(request),
	...
	public AdminUserSummary update(@PathVariable String publicId, @Valid @RequestBody UpdateInternalUserRequest body,
			@AuthenticationPrincipal AuthenticatedUser actor, HttpServletRequest request) {
		return updateInternalUserService.update(new UpdateInternalUserCommand(publicId, body.email(), body.firstName(),
				body.lastName(), body.displayName(), body.roleCode(), body.organizationPublicId(), body.startsAt(),
	...
	@PreAuthorize("hasRole('ADMINISTRATOR') and hasAuthority('USER_STATUS_CHANGE')")
	public AdminUserSummary suspend(@PathVariable String publicId, @Valid @RequestBody UserStatusChangeRequest body,
			@AuthenticationPrincipal AuthenticatedUser actor, HttpServletRequest request) {
	...
	@PreAuthorize("hasRole('ADMINISTRATOR') and hasAuthority('USER_STATUS_CHANGE')")
	public AdminUserSummary delete(@PathVariable String publicId, @Valid @RequestBody UserStatusChangeRequest body,
			@AuthenticationPrincipal AuthenticatedUser actor, HttpServletRequest request) {
