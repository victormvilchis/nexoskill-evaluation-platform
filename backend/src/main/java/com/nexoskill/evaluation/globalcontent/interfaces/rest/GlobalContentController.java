...
public class GlobalContentController {
	...
	public GlobalContentController(GlobalContentReviewService review, ContentPromotionService promotions,
			GlobalContentPublicationService publication, GlobalContentVersionService versions,
			OrganizationContentGrantService grants, ContentDistributionService distributions) {
		this.review = review;
	...
	public ReviewPage review(@RequestParam(required = false) String query,
			@RequestParam(required = false) String organizationPublicId,
			@RequestParam(required = false) GlobalContentType contentType,
			@RequestParam(required = false) ContentScope scope, @RequestParam(required = false) String status,
			@RequestParam(required = false) Long creatorUserId, @RequestParam(required = false) Instant createdFrom,
			@RequestParam(required = false) Instant createdTo, @RequestParam(required = false) Boolean promoted,
			@RequestParam(required = false) Boolean distributed, @RequestParam(defaultValue = "0") int page,
			@RequestParam(defaultValue = "10") int size, @AuthenticationPrincipal AuthenticatedUser actor) {
		return review.review(new ReviewFilter(query, organizationPublicId, contentType, scope, status, creatorUserId,
				createdFrom, createdTo, promoted, distributed, page, size), actor.internalId());
	}
	...
	public PromotionPreview preview(@RequestParam GlobalContentType type, @RequestParam String sourcePublicId) {
	...
	public PromotionView promote(@Valid @RequestBody PromotionRequest request,
		...
		return promotions.promote(
				new PromoteCommand(request.contentType(), request.sourcePublicId(), request.includeDependencies(),
						request.duplicateResolution(), request.existingGlobalPublicId(), request.notes()),
				actor.internalId());
	}

	@PostMapping("/promotions/{publicId}/submit-review")
	@PreAuthorize("hasAuthority('GLOBAL_CONTENT_PROMOTE')")
	public PromotionView submitReview(@PathVariable String publicId, @AuthenticationPrincipal AuthenticatedUser actor) {
	...
	public PromotionView publish(@PathVariable String publicId, @AuthenticationPrincipal AuthenticatedUser actor) {
	...
	@PreAuthorize("hasAuthority('GLOBAL_CONTENT_PUBLISH')")
	public PromotionView archive(@PathVariable String publicId, @AuthenticationPrincipal AuthenticatedUser actor) {
		return publication.archive(publicId, actor.internalId());
	}

	@GetMapping("/versions")
	@PreAuthorize("hasAuthority('GLOBAL_CONTENT_REVIEW')")
	public List<VersionView> versions(@RequestParam GlobalContentType type, @RequestParam String contentPublicId) {
	...
	public GrantView grant(@Valid @RequestBody GrantRequest request, @AuthenticationPrincipal AuthenticatedUser actor) {
		return grants.grant(request.toCommand(), actor.internalId());
	}

	@PostMapping("/grants/{publicId}/disable")
	@PreAuthorize("hasAuthority('GLOBAL_CONTENT_DISTRIBUTE')")
	public GrantView disableGrant(@PathVariable String publicId, @AuthenticationPrincipal AuthenticatedUser actor) {
	...
	public record PromotionRequest(@NotNull GlobalContentType contentType, @NotBlank String sourcePublicId,
			boolean includeDependencies, @NotNull DuplicateResolution duplicateResolution,
			String existingGlobalPublicId, @Size(max = 2000) String notes) {
	}

	public record ReasonRequest(@NotBlank @Size(max = 1000) String reason) {
	}

	public record VersionRequest(@NotNull GlobalContentType contentType, @NotBlank String contentPublicId,
			@Size(max = 2000) String notes) {
	}

	public record GrantRequest(@NotBlank String organizationPublicId, @NotNull GlobalContentType contentType,
			@NotBlank String globalContentPublicId, @Positive long globalVersion,
			@NotNull DistributionMode distributionMode, @NotNull AccessMode accessMode, boolean cloningAllowed,
			boolean organizationEditable, @NotNull UpdatePolicy updatePolicy, Instant availableFrom,
			Instant expiresAt) {
		GrantCommand toCommand() {
			return new GrantCommand(organizationPublicId, contentType, globalContentPublicId, globalVersion,
					distributionMode, accessMode, cloningAllowed, organizationEditable, updatePolicy, availableFrom,
					expiresAt);
		}
	}

	public record DistributionRequest(@NotNull GlobalContentType contentType, @NotBlank String globalContentPublicId,
			@Positive long globalVersion, @NotEmpty List<@NotBlank String> organizationPublicIds,
			@NotNull DistributionMode distributionMode, @NotNull AccessMode accessMode, boolean cloningAllowed,
			boolean organizationEditable, @NotNull UpdatePolicy updatePolicy, Instant availableFrom, Instant expiresAt,
			@Size(max = 2000) String notes) {
