...
public class GlobalContentPublicationService {
	...
	public GlobalContentPublicationService(ContentPromotionService promotionService,
			GlobalContentPromotionRepository promotions, GlobalContentVersionRepository versions,
			GlobalContentResourcePort resources, AuditLogPort audit, Clock clock) {
	...
	public PromotionView publish(String promotionPublicId, Long actorUserId) {
		...
		try {
			...
			resources.changeOperationalStatus(promotion.getContentType(), promotion.getGlobalContentId(), "ACTIVE",
					actorUserId);
		} catch (IllegalStateException exception) {
			throw transitionError();
		}
		promotions.save(promotion);
		audit.record(actorUserId, "GLOBAL_CONTENT_PUBLISHED", "GLOBAL_CONTENT", "Contenido global publicado.", null,
				null,
				Map.of("promotionPublicId", promotion.getPublicId(), "globalContentPublicId",
						promotion.getGlobalContentPublicId(), "globalVersion", promotion.getGlobalVersion()),
				clock.instant());
	...
	public PromotionView reject(String promotionPublicId, String reason, Long actorUserId) {
		if (reason == null || reason.isBlank()) {
			throw new BusinessException("GLOBAL_CONTENT_REJECTION_REASON_REQUIRED", "Escribe el motivo del rechazo.");
		}
		GlobalContentPromotionJpaEntity promotion = promotionService.findPromotion(promotionPublicId);
		try {
		...
		promotions.save(promotion);
		audit.record(actorUserId, "GLOBAL_CONTENT_REJECTED", "GLOBAL_CONTENT", "Promoción global rechazada.", null,
				null, Map.of("promotionPublicId", promotion.getPublicId(), "reason", reason.trim()), clock.instant());
		return promotionService.view(promotion);
	}
	...
	public PromotionView archive(String promotionPublicId, Long actorUserId) {
		...
		version.archive();
		versions.save(version);
		resources.changeOperationalStatus(promotion.getContentType(), promotion.getGlobalContentId(), "ARCHIVED",
				actorUserId);
		promotions.save(promotion);
		audit.record(actorUserId, "GLOBAL_CONTENT_ARCHIVED", "GLOBAL_CONTENT", "Contenido global archivado.", null,
				null, Map.of("promotionPublicId", promotion.getPublicId()), clock.instant());
		return promotionService.view(promotion);
	}

	private void ensureDependenciesPublished(GlobalContentPromotionJpaEntity promotion) {
		for (Dependency dependency : resources.dependencies(promotion.getContentType(),
			...
			boolean published = versions.findFirstByContentTypeAndContentIdAndStatusOrderByVersionNumberDesc(
					dependency.contentType(), dependency.internalId(), EditorialStatus.PUBLISHED).isPresent();
			if (!published) {
	...
	private GlobalContentVersionJpaEntity version(GlobalContentPromotionJpaEntity promotion) {
		return versions
				.findByContentTypeAndContentIdAndVersionNumber(promotion.getContentType(),
						promotion.getGlobalContentId(), promotion.getGlobalVersion())
