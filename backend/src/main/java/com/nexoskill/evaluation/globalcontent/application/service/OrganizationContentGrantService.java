...
public class OrganizationContentGrantService {
	...
	public OrganizationContentGrantService(OrganizationGlobalContentGrantRepository grants,
			ContentReplicationLinkRepository links, GlobalContentResourcePort resources,
			GlobalContentVersionService versions, ContentReplicationService replication,
			OrganizationRepository organizations, AuditLogPort audit, Clock clock) {
		this.grants = grants;
		this.links = links;
	...
	public GrantOutcome grantInternal(GrantCommand command, Long actorUserId) {
		...
		if (existing.isPresent()) {
			...
			grant.reconfigure(command.accessMode(), command.cloningAllowed(), command.organizationEditable(),
					command.updatePolicy(), command.availableFrom(), command.expiresAt(), actorUserId, clock.instant());
		} else {
		...
		audit.record(actorUserId, "GLOBAL_CONTENT_GRANTED", "GLOBAL_CONTENT",
				...
				Map.of("grantPublicId", grant.getPublicId(), "organizationPublicId", organization.getPublicId(),
						...
						"globalVersion", command.globalVersion(), "distributionMode",
						command.distributionMode().name()),
				clock.instant());
	...
	public boolean hasActiveReference(Long organizationId, GlobalContentType type, Long globalContentId) {
		return grants.existsByOrganizationIdAndContentTypeAndGlobalContentIdAndStatus(organizationId, type,
				globalContentId, GrantStatus.ACTIVE);
	}
	...
	private void validate(GrantCommand command) {
		...
		if (command.distributionMode() == DistributionMode.GLOBAL_REFERENCE && command.organizationEditable()) {
