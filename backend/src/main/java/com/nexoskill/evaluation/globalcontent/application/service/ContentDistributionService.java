...
public class ContentDistributionService {
    ...
    public ContentDistributionService(ContentDistributionJobRepository jobs,
            ContentDistributionResultRepository results,
            ContentDistributionWorker worker,
    ...
    public DistributionJobView distribute(DistributionCommand command, Long actorUserId) {
        ...
        ContentDistributionJobJpaEntity job = jobs.saveAndFlush(ContentDistributionJobJpaEntity.create(
                UUID.randomUUID().toString(), command.contentType(), global.internalId(), global.publicId(),
                command.globalVersion(), command.distributionMode(), resolvedDestinations.size(), actorUserId,
                trim(command.notes()), clock.instant()));
        ...
        audit.record(actorUserId, "GLOBAL_CONTENT_DISTRIBUTION_COMPLETED", "GLOBAL_CONTENT",
                ...
                Map.of("jobPublicId", job.getPublicId(), "contentType", command.contentType().name(),
                        "globalContentPublicId", global.publicId(), "globalVersion", command.globalVersion(),
                        "total", job.getTotalOrganizations(), "success", job.getSuccessfulOrganizations(),
                        "skipped", job.getSkippedOrganizations(), "failed", job.getFailedOrganizations()),
                clock.instant());
        return view(job);
    }
        ...
        List<DistributionResult> resultViews = results.findAllByJobIdOrderByProcessedAtAsc(job.getId()).stream()
                .map(result -> {
                    OrganizationJpaEntity organization = organizations.findById(result.getOrganizationId())
                            .orElse(null);
                    OrganizationGlobalContentGrantJpaEntity grant = result.getGrantId() == null
                            ? null : grants.findById(result.getGrantId()).orElse(null);
                    return new DistributionResult(
                            organization == null ? null : organization.getPublicId(),
                            organization == null ? "Organización no encontrada" : organization.getName(),
                            result.getStatus(),
                            grant == null ? null : grant.getPublicId(),
