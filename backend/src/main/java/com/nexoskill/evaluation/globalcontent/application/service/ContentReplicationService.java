...
public class ContentReplicationService {
	...
	public ContentReplicationService(GlobalContentResourcePort resources, GlobalContentVersionService versions,
			ContentReplicationLinkRepository links, AuditLogPort audit, Clock clock) {
	...
	public Optional<ReplicatedResource> existing(GlobalContentType type, Long globalContentId, long globalVersion,
			Long organizationId) {
		return links.findByOrganizationIdAndContentTypeAndSourceGlobalContentIdAndSourceGlobalVersion(organizationId,
				type, globalContentId, globalVersion).map(this::view);
	}
	...
	private ReplicatedResource replicateInternal(GlobalContentType type, Long globalContentId, long globalVersion,
		...
		Optional<ContentReplicationLinkJpaEntity> existing = links
				.findByOrganizationIdAndContentTypeAndSourceGlobalContentIdAndSourceGlobalVersion(organizationId, type,
						globalContentId, globalVersion);
		...
		try {
			...
			for (Dependency dependency : resources.dependencies(type, globalContentId)) {
				...
				long dependencyVersion = versions.latestPublishedVersion(dependency.contentType(),
						dependency.internalId());
				ReplicatedResource replicatedDependency = replicateInternal(dependency.contentType(),
						dependency.internalId(), dependencyVersion, organizationId, actorUserId, editable, stack);
				dependencyTargets.put(new ResourceKey(dependency.contentType(), dependency.internalId()),
						replicatedDependency.internalId());
			}

			ContentResource target = resources.copyToOrganization(type, globalContentId, organizationId, globalVersion,
					dependencyTargets, actorUserId, editable);
			...
			audit.record(actorUserId, "GLOBAL_CONTENT_REPLICATED", "GLOBAL_CONTENT",
					...
					Map.of("contentType", type.name(), "globalContentPublicId", global.publicId(), "globalVersion",
							globalVersion, "organizationId", organizationId, "targetContentPublicId",
							target.publicId()),
					clock.instant());
	...
	private ReplicatedResource view(ContentReplicationLinkJpaEntity link, boolean existing) {
		return new ReplicatedResource(link.getContentType(), link.getTargetContentId(), link.getTargetContentPublicId(),
				existing, link.isCustomized(), link.getSyncStatus());
	}
}
