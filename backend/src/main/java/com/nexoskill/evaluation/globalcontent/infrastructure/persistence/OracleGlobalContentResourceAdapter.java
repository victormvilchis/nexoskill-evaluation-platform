...
public class OracleGlobalContentResourceAdapter implements GlobalContentResourcePort {
	...
	public OracleGlobalContentResourceAdapter(NamedParameterJdbcTemplate jdbc, JdbcTemplate plainJdbc,
			GlobalContentPromotionRepository promotions, OrganizationGlobalContentGrantRepository grants) {
		this.jdbc = jdbc;
	...
	public ReviewPage review(ReviewFilter filter) {
		List<GlobalContentType> types = filter.contentType() == null ? List.of(GlobalContentType.CATEGORY,
				GlobalContentType.QUESTION, GlobalContentType.FORM, GlobalContentType.COLLECTION)
		...
		Long total = jdbc.queryForObject("SELECT COUNT(*) FROM (" + union + ") content " + where, parameters,
				Long.class);
		...
		parameters.addValue("offset", (long) filter.page() * filter.size()).addValue("pageSize", filter.size());
	...
	public ContentResource find(GlobalContentType type, String publicId) {
		if (type == GlobalContentType.PATH)
			unsupported(type);
	...
	public List<ContentResource> possibleGlobalDuplicates(ContentResource source) {
		return loadAll(source.contentType()).stream().filter(candidate -> candidate.scope() == ContentScope.GLOBAL)
	...
	public ContentResource copyToGlobal(GlobalContentType type, Long sourceInternalId,
		...
		case QUESTION ->
			copyQuestion(sourceInternalId, globalOrganizationId, null, null, dependencyTargets, actorUserId, true);
		case FORM -> copyForm(sourceInternalId, globalOrganizationId, null, null, dependencyTargets, actorUserId, true);
		case COLLECTION -> copyLearningCollection(sourceInternalId, globalOrganizationId, null, null, dependencyTargets,
				actorUserId, true);
		case PATH -> throw unsupported(type);
		};
	...
	public ContentResource copyToOrganization(GlobalContentType type, Long globalInternalId, Long organizationId,
		...
		Long targetId = switch (type) {
		case CATEGORY ->
			copyCategory(globalInternalId, organizationId, globalInternalId, sourceGlobalVersion, actorUserId, false);
	...
	public void markCustomized(GlobalContentType type, Long targetInternalId) {
		if (type == GlobalContentType.PATH)
			return;
		jdbc.update("UPDATE " + table(type) + " SET IS_CUSTOMIZED = 1, SYNC_STATUS = 'DIVERGED' WHERE " + idColumn(type)
				+ " = :id" + " AND SOURCE_GLOBAL_ID IS NOT NULL", Map.of("id", targetInternalId));
	}
	...
	public void changeOperationalStatus(GlobalContentType type, Long internalId, String status, Long actorUserId) {
		...
		String updatedBy = type == GlobalContentType.QUESTION || type == GlobalContentType.CATEGORY
				|| type == GlobalContentType.FORM || type == GlobalContentType.COLLECTION
						...
						: "";
		jdbc.update(
				"UPDATE " + table(type) + " SET STATUS = :status" + updatedBy + " WHERE " + idColumn(type)
						+ " = :id AND CONTENT_SCOPE = 'GLOBAL'",
				new MapSqlParameterSource().addValue("status", status).addValue("actor", actorUserId).addValue("id",
						internalId));
	}
	...
	private String reviewSelect(GlobalContentType type) {
		...
		return "SELECT '" + typeName + "' CONTENT_TYPE, source.*, " + promotedExpression(typeName) + " PROMOTED_FLAG, "
				+ distributedExpression(typeName) + " DISTRIBUTED_FLAG FROM (" + baseSelect(type) + ") source";
	}
	...
	private String distributedExpression(String typeName) {
		return "CASE WHEN (source.CONTENT_SCOPE = 'GLOBAL' AND EXISTS ("
				+ "SELECT 1 FROM ORGANIZATION_GLOBAL_CONTENT_GRANT grant_value " + "WHERE grant_value.CONTENT_TYPE = '"
				+ typeName + "' " + "AND grant_value.GLOBAL_CONTENT_ID = source.CONTENT_ID "
				+ "AND grant_value.STATUS = 'ACTIVE')) OR " + "(source.CONTENT_SCOPE = 'ORGANIZATION' AND EXISTS ("
				...
				+ "AND grant_value.STATUS = 'ACTIVE' " + "WHERE promotion.SOURCE_CONTENT_TYPE = '" + typeName + "' "
				+ "AND promotion.SOURCE_CONTENT_ID = source.CONTENT_ID "
	...
	private MapSqlParameterSource reviewParameters(ReviewFilter filter) {
		...
		if ("all".equals(status))
			status = null;
		return new MapSqlParameterSource().addValue("query", query == null ? null : "%" + query + "%")
	...
	private ContentResource mapReviewResource(ResultSet rs, int rowNum) throws SQLException {
		...
		return new ContentResource(type, id, rs.getString("PUBLIC_ID"), rs.getString("CONTENT_NAME"),
				...
				ContentScope.valueOf(rs.getString("CONTENT_SCOPE")), nullableLong(rs, "OWNER_ORGANIZATION_ID"),
				rs.getString("OWNER_ORGANIZATION_PUBLIC_ID"), rs.getString("OWNER_ORGANIZATION_NAME"),
				nullableLong(rs, "CREATED_BY"), instant(rs.getObject("CREATED_AT")), nullableLong(rs, "UPDATED_BY"),
				instant(rs.getObject("UPDATED_AT")), rs.getLong("VERSION_NO"), rs.getInt("PROMOTED_FLAG") == 1,
				rs.getInt("DISTRIBUTED_FLAG") == 1, functionalHash(type, id));
	}
	...
	private ContentResource enrich(ContentResource resource) {
		boolean promoted = promotions.findByContentTypeAndSourceContentIdAndSourceVersion(resource.contentType(),
				resource.internalId(), resource.version()).isPresent();
		boolean distributed;
		if (resource.scope() == ContentScope.GLOBAL) {
			distributed = !grants.findAllByContentTypeAndGlobalContentIdAndStatus(resource.contentType(),
					resource.internalId(), com.nexoskill.evaluation.globalcontent.domain.model.GrantStatus.ACTIVE)
					.isEmpty();
		} else {
			distributed = promotions.findFirstByContentTypeAndSourceContentIdAndStatusOrderByPromotedAtDesc(
					...
					com.nexoskill.evaluation.globalcontent.domain.model.EditorialStatus.PUBLISHED).map(
							promotion -> !grants
									.findAllByContentTypeAndGlobalContentIdAndStatus(resource.contentType(),
											promotion.getGlobalContentId(),
											com.nexoskill.evaluation.globalcontent.domain.model.GrantStatus.ACTIVE)
									.isEmpty())
					.orElse(false);
		}
	...
	private RowMapper<ContentResource> mapper(GlobalContentType type) {
		return (rs, rowNum) -> {
			...
			return new ContentResource(type, id, publicId, name, description, status, scope, ownerId,
					rs.getString("OWNER_ORGANIZATION_PUBLIC_ID"), rs.getString("OWNER_ORGANIZATION_NAME"), createdBy,
					createdAt, updatedBy, updatedAt, version, false, false, hash);
		};
	}
	...
	private List<Dependency> questionDependencies(Long questionId) {
		String sql = """
				...
				""";
		return jdbc.query(sql, Map.of("id", questionId),
				(rs, rowNum) -> dependency(GlobalContentType.CATEGORY, rs.getLong("CONTENT_ID"),
						rs.getString("PUBLIC_ID"), rs.getString("CONTENT_NAME"),
						ContentScope.valueOf(rs.getString("CONTENT_SCOPE")), nullableLong(rs, "OWNER_ORGANIZATION_ID"),
	...
	private List<Dependency> formDependencies(Long formId) {
		...
		String questionSql = """
				SELECT DISTINCT q.QUESTION_ID CONTENT_ID, q.PUBLIC_ID,
				 ...
				 WHERE s.FORM_ID = :id
				""";
		result.addAll(jdbc.query(questionSql, Map.of("id", formId),
				(rs, rowNum) -> dependency(GlobalContentType.QUESTION, rs.getLong("CONTENT_ID"),
						rs.getString("PUBLIC_ID"), rs.getString("CONTENT_NAME"),
						ContentScope.valueOf(rs.getString("CONTENT_SCOPE")), nullableLong(rs, "OWNER_ORGANIZATION_ID"),
						rs.getLong("VERSION_NO"))));
		String categorySql = """
				SELECT DISTINCT c.CATEGORY_ID CONTENT_ID, c.PUBLIC_ID, c.CATEGORY_NAME CONTENT_NAME,
				 ...
				 WHERE s.FORM_ID = :id
				""";
		result.addAll(jdbc.query(categorySql, Map.of("id", formId),
				(rs, rowNum) -> dependency(GlobalContentType.CATEGORY, rs.getLong("CONTENT_ID"),
						rs.getString("PUBLIC_ID"), rs.getString("CONTENT_NAME"),
						ContentScope.valueOf(rs.getString("CONTENT_SCOPE")), nullableLong(rs, "OWNER_ORGANIZATION_ID"),
						rs.getLong("VERSION_NO"))));
	...
	private List<Dependency> collectionDependencies(Long collectionId) {
		String sql = """
				SELECT DISTINCT f.FORM_ID CONTENT_ID, f.PUBLIC_ID, f.TITLE CONTENT_NAME,
				 ...
				 ORDER BY f.TITLE
				""";
		return jdbc.query(sql, Map.of("id", collectionId),
				(rs, rowNum) -> dependency(GlobalContentType.FORM, rs.getLong("CONTENT_ID"), rs.getString("PUBLIC_ID"),
	...
	private Dependency dependency(GlobalContentType type, Long id, String publicId, String name, ContentScope scope,
			Long ownerOrganizationId, long version) {
		Optional<GlobalContentPromotionJpaEntity> promotion = scope == ContentScope.GLOBAL ? Optional.empty()
		...
		return new Dependency(type, id, publicId, name, scope, ownerOrganizationId, version,
				scope == ContentScope.GLOBAL || promotion.isPresent(), scope == ContentScope.GLOBAL ? publicId
						: promotion.map(GlobalContentPromotionJpaEntity::getGlobalContentPublicId).orElse(null));
	}

	private Long copyCategory(Long sourceId, Long ownerOrganizationId, Long sourceGlobalId, Long sourceGlobalVersion,
			Long actor, boolean global) {
		...
		MapSqlParameterSource parameters = new MapSqlParameterSource().addValue("publicId", publicId)
				.addValue("code", code).addValue("name", source.get("CATEGORY_NAME"))
	...
	private Long copyQuestion(Long sourceId, Long ownerOrganizationId, Long sourceGlobalId, Long sourceGlobalVersion,
			Map<ResourceKey, Long> dependencyTargets, Long actor, boolean global) {
		Map<String, Object> source = requiredRow("SELECT * FROM QUESTION WHERE QUESTION_ID = :id", sourceId);
		MapSqlParameterSource parameters = new MapSqlParameterSource()
				.addValue("publicId", UUID.randomUUID().toString()).addValue("typeCode", source.get("TYPE_CODE"))
				.addValue("difficultyCode", source.get("DIFFICULTY_CODE"))
				.addValue("technologyId", source.get("TECHNOLOGY_ID")).addValue("levelCode", source.get("LEVEL_CODE"))
				.addValue("status", "ACTIVE").addValue("statement", source.get("STATEMENT_TEXT"))
				...
				.addValue("manualReview", source.get("MANUAL_REVIEW")).addValue("numericMin", source.get("NUMERIC_MIN"))
		...
		for (Long categoryId : categoryIds) {
			Long targetCategoryId = dependencyTargets
					.getOrDefault(new ResourceKey(GlobalContentType.CATEGORY, categoryId), categoryId);
			jdbc.update(
					"INSERT INTO QUESTION_CATEGORY_RELATION (QUESTION_ID, CATEGORY_ID) VALUES (:questionId, :categoryId)",
		...
		for (Map<String, Object> option : options) {
			MapSqlParameterSource optionParams = new MapSqlParameterSource()
					.addValue("publicId", UUID.randomUUID().toString()).addValue("questionId", targetId)
					.addValue("order", option.get("OPTION_ORDER")).addValue("text", option.get("OPTION_TEXT"))
					.addValue("media", option.get("MEDIA_ID")).addValue("matchText", option.get("MATCH_TEXT"))
					.addValue("matchMedia", option.get("MATCH_MEDIA_ID")).addValue("correct", option.get("IS_CORRECT"))
	...
	private Long copyForm(Long sourceId, Long ownerOrganizationId, Long sourceGlobalId, Long sourceGlobalVersion,
			Map<ResourceKey, Long> dependencyTargets, Long actor, boolean global) {
		Map<String, Object> source = requiredRow("SELECT * FROM EVALUATION_FORM WHERE FORM_ID = :id", sourceId);
		String code = uniqueCode(GlobalContentType.FORM, Objects.toString(source.get("FORM_CODE")), ownerOrganizationId,
				global);
		MapSqlParameterSource parameters = new MapSqlParameterSource()
				...
				.addValue("showResults", source.get("SHOW_RESULTS"))
				.addValue("showCorrect", source.get("SHOW_CORRECT_ANSWERS"))
				.addValue("randomQuestions", source.get("RANDOMIZE_QUESTIONS"))
				.addValue("randomOptions", source.get("RANDOMIZE_OPTIONS"))
				.addValue("showProgress", source.get("SHOW_PROGRESS"))
				.addValue("hideNumbers", source.get("HIDE_QUESTION_NUMBERS"))
				.addValue("saveResume", source.get("ALLOW_SAVE_RESUME"))
				.addValue("oneAttempt", source.get("ONE_ACTIVE_ATTEMPT"))
		...
		for (Map<String, Object> section : sections) {
			...
			MapSqlParameterSource sectionParams = new MapSqlParameterSource().addValue("formId", targetId)
					.addValue("publicId", UUID.randomUUID().toString()).addValue("title", section.get("TITLE"))
					.addValue("description", section.get("DESCRIPTION"))
			...
			for (Map<String, Object> item : fixed) {
				...
				Long targetQuestionId = dependencyTargets
						.getOrDefault(new ResourceKey(GlobalContentType.QUESTION, sourceQuestionId), sourceQuestionId);
				jdbc.update("""
			...
			for (Map<String, Object> pool : pools) {
				Long categoryId = pool.get("CATEGORY_ID") == null ? null
						: ((Number) pool.get("CATEGORY_ID")).longValue();
				if (categoryId != null) {
					categoryId = dependencyTargets.getOrDefault(new ResourceKey(GlobalContentType.CATEGORY, categoryId),
							categoryId);
				}
				jdbc.update("""
						...
						new MapSqlParameterSource().addValue("section", targetSectionId)
								...
								.addValue("collectionId", pool.get("COLLECTION_ID")).addValue("categoryId", categoryId)
								.addValue("count", pool.get("QUESTION_COUNT"))
	...
	private Long copyLearningCollection(Long sourceId, Long ownerOrganizationId, Long sourceGlobalId,
		...
		Map<String, Object> source = requiredRow("SELECT * FROM LEARNING_COLLECTION WHERE COLLECTION_ID = :id",
				sourceId);
		...
		for (Map<String, Object> level : levels) {
			...
			Long targetFormId = dependencyTargets.getOrDefault(new ResourceKey(GlobalContentType.FORM, sourceFormId),
					sourceFormId);
			jdbc.update("""
	...
	private String uniqueCode(GlobalContentType type, String sourceCode, Long ownerOrganizationId, boolean global) {
		String base = sourceCode == null || sourceCode.isBlank() ? type.name()
	...
	private boolean codeExists(GlobalContentType type, String code, Long ownerOrganizationId, boolean global) {
		...
		case CATEGORY ->
			"SELECT COUNT(*) FROM QUESTION_CATEGORY WHERE UPPER(CATEGORY_CODE)=UPPER(:code) AND CONTENT_SCOPE=:scope AND NVL(OWNER_ORGANIZATION_ID,-1)=NVL(:owner,-1)";
		case FORM ->
			"SELECT COUNT(*) FROM EVALUATION_FORM WHERE UPPER(FORM_CODE)=UPPER(:code) AND CONTENT_SCOPE=:scope AND NVL(OWNER_ORGANIZATION_ID,-1)=NVL(:owner,-1)";
		case COLLECTION ->
			"SELECT COUNT(*) FROM LEARNING_COLLECTION WHERE UPPER(COLLECTION_CODE)=UPPER(:code) AND CONTENT_SCOPE=:scope AND NVL(OWNER_ORGANIZATION_ID,-1)=NVL(:owner,-1)";
		default -> null;
		};
		if (sql == null)
			return false;
		Integer count = jdbc.queryForObject(
				sql, new MapSqlParameterSource().addValue("code", code)
						.addValue("scope", global ? "GLOBAL" : "ORGANIZATION").addValue("owner", ownerOrganizationId),
				Integer.class);
	...
	private Long insert(String sql, MapSqlParameterSource parameters, String keyColumn) {
		...
		if (key == null) {
			throw new BusinessException("GLOBAL_CONTENT_COPY_FAILED",
					"No fue posible obtener el identificador de la copia creada.");
		}
