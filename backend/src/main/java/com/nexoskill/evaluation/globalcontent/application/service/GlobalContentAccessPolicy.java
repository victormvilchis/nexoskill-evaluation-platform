...
/**
 * Centraliza las reglas de lectura y edición de contenido GLOBAL y
 * ORGANIZATION.
 */
@Component
public class GlobalContentAccessPolicy {
	...
	public GlobalContentAccessPolicy(OrganizationRepository organizations,
			OrganizationGlobalContentGrantRepository grants, ContentReplicationLinkRepository replicationLinks,
			GlobalContentVersionRepository versions, Clock clock) {
	...
	public GlobalContentAccessPolicy(OrganizationRepository organizations,
			OrganizationGlobalContentGrantRepository grants, ContentReplicationLinkRepository replicationLinks,
			GlobalContentVersionRepository versions, Clock clock, NamedParameterJdbcTemplate jdbc) {
	...
	public boolean canRead(GlobalContentType type, Long internalId, ContentScope scope, Long ownerOrganizationId,
			TenantContext tenant) {
		if (tenant == null || scope == null || internalId == null)
			return false;
		...
		if (organization == null || organization.isGlobal())
			return false;
		if (!isPublishedOrLegacy(type, internalId))
			return false;
		if (organization.getContentMode() == ContentMode.GLOBAL_CATALOG)
			return true;
		Instant now = clock.instant();
		return grants
				.findAllByOrganizationIdAndContentTypeAndGlobalContentIdAndStatus(tenant.organizationId(), type,
						internalId, GrantStatus.ACTIVE)
				.stream().anyMatch(grant -> grant.getDistributionMode() == DistributionMode.GLOBAL_REFERENCE
						&& grant.isOperational(now));
	}

	private boolean questionAvailabilityAllows(Long questionId, Long organizationId) {
		if (jdbc == null || organizationId == null)
			return false;
	...
	private boolean isPublishedOrLegacy(GlobalContentType type, Long internalId) {
		if (!versions.existsByContentTypeAndContentId(type, internalId))
			return true;
		return versions.findFirstByContentTypeAndContentIdAndStatusOrderByVersionNumberDesc(type, internalId,
				EditorialStatus.PUBLISHED).isPresent();
	}

	public void assertReadable(GlobalContentType type, Long internalId, ContentScope scope, Long ownerOrganizationId,
			TenantContext tenant, String notFoundCode, String notFoundMessage) {
		if (!canRead(type, internalId, scope, ownerOrganizationId, tenant)) {
			throw new BusinessException(notFoundCode, notFoundMessage);
		}
	}

	public void assertEditable(GlobalContentType type, Long internalId, ContentScope scope, Long ownerOrganizationId,
			Long sourceGlobalId, TenantContext tenant) {
		...
		if (sourceGlobalId == null)
			return;
		var link = replicationLinks
				.findByOrganizationIdAndContentTypeAndTargetContentId(tenant.organizationId(), type, internalId)
				.orElse(null);
		if (link == null) {
			// Las copias creadas explícitamente desde el módulo funcional conservan
			// SOURCE_GLOBAL_ID
			// y siguen siendo editables únicamente por su organización propietaria.
			return;
		}
		boolean editable = grants
				.findAllByOrganizationIdAndContentTypeAndGlobalContentIdAndStatus(tenant.organizationId(), type,
						sourceGlobalId, GrantStatus.ACTIVE)
	...
	public boolean allowsAllGlobal(TenantContext tenant) {
		...
		return organizations.findById(tenant.organizationId())
				.map(organization -> organization.getContentMode() == ContentMode.GLOBAL_CATALOG).orElse(false);
	}
