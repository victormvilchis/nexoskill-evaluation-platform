...
public class ContentPromotionService {
    ...
    public ContentPromotionService(GlobalContentResourcePort resources,
    ...
    private PromotionView promoteInternal(PromoteCommand command, Long actorUserId,
            Set<ResourceKey> stack, Map<ResourceKey, GlobalContentPromotionJpaEntity> promotedDuringOperation) {
        ...
        try {
            Optional<GlobalContentPromotionJpaEntity> existing = promotions
                    .findByContentTypeAndSourceContentIdAndSourceVersion(
                            source.contentType(), source.internalId(), source.version());
            if (existing.isPresent()) return view(existing.get());
            if (promotedDuringOperation.containsKey(sourceKey)) return view(promotedDuringOperation.get(sourceKey));
            ...
            for (Dependency dependency : dependencies) {
                ...
                Optional<GlobalContentPromotionJpaEntity> dependencyPromotion = promotions
                        .findByContentTypeAndSourceContentIdAndSourceVersion(
                                dependency.contentType(), dependency.internalId(), dependency.version());
                if (dependencyPromotion.isEmpty()) {
                    ...
                    PromotionView created = promoteInternal(new PromoteCommand(dependency.contentType(),
                                    dependency.publicId(), true, DuplicateResolution.CREATE_DISTINCT, null,
            ...
            DuplicateResolution resolution = command.duplicateResolution() == null
                    ? DuplicateResolution.CREATE_DISTINCT : command.duplicateResolution();
            ...
            if (resolution == DuplicateResolution.USE_EXISTING) {
                ...
                globalVersion = versions.findFirstByContentTypeAndContentIdOrderByVersionNumberDesc(
                                source.contentType(), global.internalId())
                ...
                global = resources.copyToGlobal(source.contentType(), source.internalId(), dependencyTargets,
                        actorUserId);
                if (resolution == DuplicateResolution.CREATE_NEW_VERSION
                        && command.existingGlobalPublicId() != null && !command.existingGlobalPublicId().isBlank()) {
                    ContentResource lineage = resources.find(source.contentType(), command.existingGlobalPublicId());
                    globalVersion = versions.findFirstByContentTypeAndContentIdOrderByVersionNumberDesc(
                                    source.contentType(), lineage.internalId())
                            .map(value -> value.getVersionNumber() + 1)
                            .orElse(Math.max(1, lineage.version()) + 1);
                } else {
                    globalVersion = 1L;
                }
            }

            var promotion = promotions.saveAndFlush(GlobalContentPromotionJpaEntity.create(
                    UUID.randomUUID().toString(), source.ownerOrganizationId(), source.contentType(),
                    source.internalId(), source.publicId(), source.version(), source.createdBy(),
                    global.internalId(), global.publicId(), globalVersion, trim(command.notes()),
                    source.functionalHash(), actorUserId, clock.instant()));
            ...
            audit.record(actorUserId, "GLOBAL_CONTENT_PROMOTED", "GLOBAL_CONTENT",
                    ...
                    Map.of("promotionPublicId", promotion.getPublicId(), "contentType", source.contentType().name(),
                            "sourceOrganizationId", source.ownerOrganizationId(),
                            "sourceContentPublicId", source.publicId(),
                            "globalContentPublicId", global.publicId(), "globalVersion", version.getVersionNumber()),
                    clock.instant());
    ...
    private void validateSource(ContentResource source) {
        ...
        if ("DELETED".equalsIgnoreCase(source.status())) {
            throw new BusinessException("GLOBAL_CONTENT_SOURCE_DELETED",
                    "No se puede promover contenido eliminado.");
        }
    }
    ...
    PromotionView view(GlobalContentPromotionJpaEntity promotion) {
        ...
        OrganizationJpaEntity organization = organizations.findById(promotion.getSourceOrganizationId())
                .orElseThrow(() -> new BusinessException("ORGANIZATION_NOT_FOUND",
                        "La organización de origen ya no existe."));
