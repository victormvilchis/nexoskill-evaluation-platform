...
public class LearningCollectionService {
	...
	public List<LearningCollectionModels.CollectionSummary> list(String query, String status) {

		String normalizedQuery = normalizeSearch(query);
		String normalizedStatus = normalizeOptionalStatus(status);

		return collectionRepository.findAll().stream()
				.filter(collection -> normalizedStatus == null || normalizedStatus.equals(collection.status))
				.filter(collection -> normalizedQuery == null || searchableText(collection).contains(normalizedQuery))
				.sorted(Comparator.comparing(collection -> collection.name.toLowerCase(Locale.ROOT))).map(this::summary)
				.toList();
	}
	...
	public List<LearningCollectionModels.FormOption> formOptions(String query, String status) {
		...
		return formRepository.findAll().stream().filter(form -> !"ARCHIVED".equals(form.status))
				.filter(form -> normalizedStatus == null || normalizedStatus.equals(form.status))
				.filter(form -> normalizedQuery == null || formSearchableText(form).contains(normalizedQuery))
	...
	private static String searchableText(LearningCollectionJpaEntity collection) {
		return normalizeForSearch(
				collection.name + " " + collection.code + " " + Objects.toString(collection.description, ""));
	}
