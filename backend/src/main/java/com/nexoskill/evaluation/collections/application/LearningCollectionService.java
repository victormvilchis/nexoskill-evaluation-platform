...
public class LearningCollectionService {
	...
	public LearningCollectionService(LearningCollectionRepository collectionRepository, FormRepository formRepository,
			TenantContextResolver tenantContextResolver, GlobalContentAccessPolicy accessPolicy,
			ContentSynchronizationService synchronization, HttpServletRequest request) {
	...
	public Page<LearningCollectionModels.CollectionSummary> list(String query, String status, Pageable pageable) {

		String normalizedQuery = normalizeSearch(query);
		String normalizedStatus = normalizeOptionalStatus(status);
		var tenant = tenantContextResolver.resolve(request);
		return collectionRepository.searchVisible(normalizedQuery == null ? null : "%" + normalizedQuery + "%",
				normalizedStatus, tenant.globalScope(), tenant.organizationId(), accessPolicy.allowsAllGlobal(tenant),
				ContentScope.ORGANIZATION, ContentScope.GLOBAL, GlobalContentType.COLLECTION, GrantStatus.ACTIVE,
				DistributionMode.GLOBAL_REFERENCE, EditorialStatus.PUBLISHED, Instant.now(), pageable)
				.map(this::summary);
	}

	@Transactional(readOnly = true)
	public LearningCollectionModels.CollectionDetail get(String publicId) {
		LearningCollectionJpaEntity collection = collectionRepository.findByPublicId(canonicalUuid(publicId))
				.orElseThrow(() -> new BusinessException("COLLECTION_NOT_FOUND", "La colección solicitada no existe."));
	...
	@Transactional(readOnly = true)
	public List<LearningCollectionModels.FormOption> formOptions(String query, String status) {
		...
		return formRepository
				.searchVisible(normalizedQuery == null ? null : "%" + normalizedQuery + "%", normalizedStatus, null,
						tenant.globalScope(), tenant.organizationId(), accessPolicy.allowsAllGlobal(tenant),
						ContentScope.ORGANIZATION, ContentScope.GLOBAL, GlobalContentType.FORM, GrantStatus.ACTIVE,
						DistributionMode.GLOBAL_REFERENCE, EditorialStatus.PUBLISHED, Instant.now(),
						Pageable.unpaged(Sort.by(Sort.Direction.ASC, "title")))
				.stream().filter(form -> !"ARCHIVED".equals(form.status))
				.map(form -> new LearningCollectionModels.FormOption(form.publicId, form.code, form.title, form.status,
						form.modeCode, form.passingScore, form.durationMinutes))
				.toList();
	}

	@Transactional
	public LearningCollectionModels.CollectionDetail create(LearningCollectionModels.CollectionCommand command,
			Long actorUserId) {
	...
	@Transactional
	public LearningCollectionModels.CollectionDetail update(String publicId,
			LearningCollectionModels.CollectionCommand command, Long actorUserId) {

		validateCommand(command);

		LearningCollectionJpaEntity collection = collectionRepository.findByPublicIdForUpdate(canonicalUuid(publicId))
				.orElseThrow(() -> new BusinessException("COLLECTION_NOT_FOUND", "La colección solicitada no existe."));
		assertEditable(collection);

		if (command.version() != null && !Objects.equals(command.version(), collection.version)) {
			throw new BusinessException("COLLECTION_CONCURRENT_MODIFICATION",
					"La colección fue modificada por otra sesión. Recarga la página e intenta nuevamente.");
		}

		if ("ARCHIVED".equals(collection.status)) {
			throw new BusinessException("COLLECTION_ARCHIVED", "Una colección archivada no puede modificarse.");
		}

		ensureUniqueName(command.name(), collection.publicId);
		OffsetDateTime now = OffsetDateTime.now();
		List<LearningCollectionLevelJpaEntity> newLevels = buildLevels(command.formPublicIds(), now);
	...
	@Transactional
	public LearningCollectionModels.CollectionDetail changeStatus(String publicId, String targetStatus,
			Long actorUserId) {
	...
	private void assertReadable(LearningCollectionJpaEntity collection) {
		accessPolicy.assertReadable(GlobalContentType.COLLECTION, collection.id, collection.contentScope,
				collection.ownerOrganizationId, tenantContextResolver.resolve(request), "COLLECTION_NOT_FOUND",
	...
	private void markCustomized(LearningCollectionJpaEntity collection, OffsetDateTime now) {
		if (collection.sourceGlobalId == null)
			return;
	...
	private void validateActivationReadiness(List<LearningCollectionLevelJpaEntity> levels) {

		boolean containsUnavailableForm = levels.stream().anyMatch(level -> !"ACTIVE".equals(level.form.status));

		if (containsUnavailableForm) {
			throw new BusinessException("COLLECTION_CONTAINS_INACTIVE_FORM",
	...
	private void validateCommand(LearningCollectionModels.CollectionCommand command) {
		if (command == null || command.name() == null || command.name().trim().length() < 3) {
			throw new BusinessException("COLLECTION_NAME_REQUIRED",
					"Escribe un nombre de al menos 3 caracteres para la colección.");
		}

		if (command.name().trim().length() > 200) {
			throw new BusinessException("COLLECTION_NAME_TOO_LONG",
		...
		if (command.description() != null && command.description().trim().length() > 2000) {
			throw new BusinessException("COLLECTION_DESCRIPTION_TOO_LONG",
		...
		for (String publicId : command.formPublicIds()) {
			...
			if (!uniqueIds.add(canonicalId)) {
				throw new BusinessException("COLLECTION_FORM_DUPLICATED",
						"Un formulario no puede repetirse dentro de la misma colección.");
			}
		}
	}

	private List<LearningCollectionLevelJpaEntity> buildLevels(List<String> publicIds, OffsetDateTime now) {
		...
		for (int index = 0; index < publicIds.size(); index++) {
			...
			FormJpaEntity form = formRepository.findByPublicId(formPublicId)
					.orElseThrow(() -> new BusinessException("COLLECTION_FORM_NOT_FOUND",
							"Uno de los formularios seleccionados ya no está disponible."));

			assertReadable(form);

			if ("ARCHIVED".equals(form.status)) {
				throw new BusinessException("COLLECTION_FORM_ARCHIVED",
	...
	private LearningCollectionModels.CollectionSummary summary(LearningCollectionJpaEntity collection) {

		collection.levels.size();
		int activeLevelCount = (int) collection.levels.stream().filter(level -> "ACTIVE".equals(level.form.status))
				.count();

		return new LearningCollectionModels.CollectionSummary(collection.publicId, collection.code, collection.name,
				collection.description, collection.status, collection.levels.size(), activeLevelCount,
				collection.updatedAt, collection.version);
	}

	private LearningCollectionModels.CollectionDetail detail(LearningCollectionJpaEntity collection) {
		...
		List<LearningCollectionModels.LevelView> levels = collection.levels.stream()
				...
				.map(level -> new LearningCollectionModels.LevelView(level.levelOrder, level.unlockRule,
						level.form.publicId, level.form.code, level.form.title, level.form.status, level.form.modeCode,
						level.form.passingScore, level.form.durationMinutes))
				.toList();

		return new LearningCollectionModels.CollectionDetail(collection.publicId, collection.code, collection.name,
				collection.description, collection.status, levels, collection.createdAt, collection.updatedAt,
				collection.version);
	}
	...
	private void ensureUniqueName(String name, String currentPublicId) {
		...
		boolean duplicated = collectionRepository.findAll().stream()
				.filter(collection -> currentPublicId == null || !currentPublicId.equals(collection.publicId))
				.map(collection -> normalizeForSearch(collection.name).replaceAll("\\s+", " "))
				.anyMatch(normalizedName::equals);

		if (duplicated) {
			throw new BusinessException("COLLECTION_ALREADY_EXISTS", "Ya existe una colección con ese nombre.");
		}
	}
	...
	private static String canonicalUuid(String value) {
		try {
			...
			throw new BusinessException("INVALID_PUBLIC_ID", "El identificador recibido no es válido.");
		}
	}
	...
	private static String normalizeRequiredStatus(String status) {
		...
		if (!VALID_STATUSES.contains(normalized)) {
			throw new BusinessException("COLLECTION_STATUS_INVALID",
	...
	private static String normalizeOptionalFormStatus(String status) {
		...
		if (!Set.of("DRAFT", "ACTIVE", "DISABLED", "CLOSED").contains(normalized)) {
			throw new BusinessException("FORM_STATUS_INVALID", "El filtro de estado del formulario no es válido.");
		}
		return normalized;
	}
}
