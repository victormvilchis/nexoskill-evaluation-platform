...
public class TalentCvService {
    ...
    public CvMetadata upload(TenantContext tenant, String publicId, MultipartFile file, StudentService.Actor actor) {
        ...
        TalentCvJpaEntity document = documents.findByStudentId(talent.getId())
                .orElseGet(() -> TalentCvJpaEntity.create(UUID.randomUUID().toString(), talent.getId(),
                        talent.getOrganizationId(), originalName, contentType(file, extension), extension,
                        data, actor.userId(), now));
        ...
        audit.record(actor.userId(), "TALENT_CV_UPDATED", "TALENT_BANK",
                "El CV del talento fue actualizado.", actor.ipAddress(), actor.userAgent(),
                Map.of("studentPublicId", talent.getPublicId(), "organizationId", talent.getOrganizationId(),
                        "fileExtension", extension, "fileSize", data.length), now);
    ...
    public CvDownload download(TenantContext tenant, String publicId) {
        StudentJpaEntity talent = students.findScopedForUpdate(tenant, publicId, StudentRecordModule.TALENT_BANK);
        TalentCvJpaEntity document = documents.findByStudentIdAndOrganizationId(talent.getId(), talent.getOrganizationId())
                .orElseThrow(() -> new BusinessException("TALENT_CV_NOT_FOUND", "El talento no tiene un CV registrado."));
    ...
    public record CvMetadata(String publicId, String fileName, String contentType, String extension,
            long fileSize, Instant updatedAt) {}
    public record CvDownload(String fileName, String contentType, byte[] data) {
        public CvDownload { data = data == null ? new byte[0] : data.clone(); }
    }
}
