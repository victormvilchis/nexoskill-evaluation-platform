...
/**
 * Lector XLSX acotado al flujo de importación de colaboradores. No ejecuta
 * fórmulas, no procesa macros y utiliza exclusivamente la primera hoja del
 * libro, sin depender de su nombre.
 */
@Component
public class XlsxCertificationReader {
	static final long MAX_UNCOMPRESSED_BYTES = 100L * 1024L * 1024L;
	...
	private static final Set<String> REQUIRED_HEADERS = Set.of(COLLABORATOR_NAME_HEADER, normalizeHeader("PERFIL"),
			normalizeHeader("FECHA DE ALTA"), normalizeHeader("TECNOLOGÍA EN LA QUE SE CERTIFICA"),
	...
	private boolean isRelevantEntry(String name) {
		return "xl/workbook.xml".equals(name) || "xl/_rels/workbook.xml.rels".equals(name)
				|| "xl/sharedStrings.xml".equals(name) || name.startsWith("xl/worksheets/");
	}
	...
	private SheetRef findFirstSheet(byte[] xml, Map<String, String> relationships) throws XMLStreamException {
		XMLStreamReader reader = xmlReader(xml);
		try {
			while (reader.hasNext()) {
		...
		throw new BusinessException("STUDENT_IMPORT_SHEET_REQUIRED", "El archivo no contiene hojas procesables.");
	}
	...
	private SheetData parseSheet(String sheetName, byte[] xml, List<String> sharedStrings) throws XMLStreamException {
			...
			while (reader.hasNext()) {
				...
				if (event == XMLStreamConstants.START_ELEMENT && "row".equals(reader.getLocalName())) {
					...
					if (column >= 0 && column < MAX_COLUMNS) {
						...
						&& cells != null) {
					if (!cells.isEmpty())
						rows.add(new RawRow(currentRow, Map.copyOf(cells)));
		...
		List<String> missing = REQUIRED_HEADERS.stream().filter(required -> !headers.containsKey(required))
				.map(XlsxCertificationReader::displayRequiredHeader).sorted().toList();
		if (!missing.isEmpty()) {
		...
		for (int index = headerPosition + 1; index < rows.size(); index++) {
			...
			headers.forEach(
					(normalized, ref) -> values.put(normalized, row.cells().getOrDefault(ref.column(), "").trim()));
	...
	private int findHeaderPosition(List<RawRow> rows) {
		...
		for (int index = 0; index < limit; index++) {
			Set<String> normalized = rows.get(index).cells().values().stream()
					.map(XlsxCertificationReader::normalizeHeader).collect(java.util.stream.Collectors.toSet());
			int matches = (int) REQUIRED_HEADERS.stream().filter(normalized::contains).count();
			if (matches > bestMatches) {
				bestMatches = matches;
				bestIndex = index;
			}
			if (matches == REQUIRED_HEADERS.size())
				return index;
		}
		if (bestIndex >= 0 && bestMatches >= 3)
			return bestIndex;
	...
	private String readCell(XMLStreamReader reader, String type, List<String> sharedStrings) throws XMLStreamException {
		String value = null;
	...
	private static void skipElement(XMLStreamReader reader, String localName) throws XMLStreamException {
		...
		while (reader.hasNext() && depth > 0) {
			...
			if (event == XMLStreamConstants.START_ELEMENT && localName.equals(reader.getLocalName()))
				depth++;
			if (event == XMLStreamConstants.END_ELEMENT && localName.equals(reader.getLocalName()))
				depth--;
		}
	}
	...
	public static String normalizeHeader(String value) {
		...
		String decomposed = Normalizer.normalize(value, Normalizer.Form.NFD).replaceAll("\\p{M}+", "")
				.toUpperCase(Locale.ROOT).replace('¿', ' ').replace('?', ' ');
	...
	public record SheetData(String sheetName, int headerRow, Map<String, String> headers, List<RowData> rows) {
	}
}
