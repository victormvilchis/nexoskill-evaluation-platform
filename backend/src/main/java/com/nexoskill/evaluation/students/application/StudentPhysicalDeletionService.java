...
public class StudentPhysicalDeletionService {
    ...
    public PageResult search(TenantContext tenant, String query, String module, String organizationPublicId,
            int page, int size, Set<String> permissions) {
        requireTenant(tenant);
        requireDeletionPermission(module, permissions);
        ...
        if (module == null || module.isBlank() || "ALL".equalsIgnoreCase(module)) {
            boolean students = permissions != null && permissions.contains("STUDENT_DELETE");
            boolean talents = permissions != null && permissions.contains("TALENT_DELETE");
            if (students && !talents) where.append(" AND s.RECORD_MODULE='COLLABORATOR' ");
            if (talents && !students) where.append(" AND s.RECORD_MODULE='TALENT_BANK' ");
        }
        ...
        Long total = jdbc.queryForObject("SELECT COUNT(*) FROM STUDENT s JOIN ORGANIZATION o ON o.ORGANIZATION_ID=s.ORGANIZATION_ID " + where,
                params, Long.class);
        List<Candidate> content = total == null || total == 0 ? List.of() : jdbc.query("""
            ...
            """ + where + " ORDER BY LOWER(s.DISPLAY_NAME), s.STUDENT_ID OFFSET :offset ROWS FETCH NEXT :size ROWS ONLY",
                params, (rs, rowNum) -> new Candidate(rs.getString("PUBLIC_ID"), rs.getString("DISPLAY_NAME"),
                        rs.getString("EMAIL"), rs.getString("STATUS"), rs.getString("RECORD_MODULE"), rs.getString("TALENT_TYPE"),
                        rs.getString("ORGANIZATION_PUBLIC_ID"), rs.getString("ORGANIZATION_CODE"),
                        rs.getString("ORGANIZATION_NAME"), rs.getBoolean("HAS_CV")));
    ...
    @Transactional
    public DeletionResult delete(TenantContext tenant, String publicId, boolean confirmed,
            StudentService.Actor actor, Set<String> permissions) {
        if (!confirmed) {
        ...
        String sql = """
            ...
            """ + ((!tenant.globalAdministrator() || !tenant.globalScope()) ? " AND s.ORGANIZATION_ID=:organizationId" : "")
                + " FOR UPDATE";
        if (!tenant.globalAdministrator() || !tenant.globalScope()) params.addValue("organizationId", tenant.organizationId());
        ...
        if (rows.isEmpty()) throw new BusinessException("PERMANENT_DELETION_NOT_FOUND", "El registro no existe o no pertenece a tu organización.");
        StudentRef student = rows.getFirst();
        requireDeletionPermission(student.module(), permissions);
        MapSqlParameterSource ids = new MapSqlParameterSource("studentId", student.id())
                .addValue("publicId", publicId);

        jdbc.update("DELETE FROM STUDENT_CERTIFICATION_HISTORY WHERE STUDENT_ID=:studentId", ids);
        jdbc.update("DELETE FROM STUDENT_CERTIFICATION_CYCLE_ATTEMPT WHERE STUDENT_CERTIFICATION_CYCLE_ID IN (SELECT STUDENT_CERTIFICATION_CYCLE_ID FROM STUDENT_CERTIFICATION_CYCLE WHERE STUDENT_ID=:studentId)", ids);
        jdbc.update("DELETE FROM STUDENT_CERTIFICATION_ATTEMPT WHERE STUDENT_CERT_REQUIREMENT_ID IN (SELECT r.STUDENT_CERT_REQUIREMENT_ID FROM STUDENT_CERT_REQUIREMENT r JOIN STUDENT_CERTIFICATION_PROFILE p ON p.STUDENT_CERTIFICATION_PROFILE_ID=r.STUDENT_CERTIFICATION_PROFILE_ID WHERE p.STUDENT_ID=:studentId)", ids);
        jdbc.update("DELETE FROM STUDENT_CERTIFICATION_CYCLE WHERE STUDENT_ID=:studentId", ids);
        jdbc.update("DELETE FROM STUDENT_CERT_REQUIREMENT WHERE STUDENT_CERTIFICATION_PROFILE_ID IN (SELECT STUDENT_CERTIFICATION_PROFILE_ID FROM STUDENT_CERTIFICATION_PROFILE WHERE STUDENT_ID=:studentId)", ids);
        ...
        jdbc.update("DELETE FROM AUDIT_EVENT WHERE EVENT_DATA IS NOT NULL AND DBMS_LOB.INSTR(EVENT_DATA, :publicId) > 0", ids);
        int deleted = jdbc.update("DELETE FROM STUDENT WHERE STUDENT_ID=:studentId", ids);
        if (deleted != 1) throw new BusinessException("PERMANENT_DELETION_FAILED", "No fue posible eliminar el registro completo.");
        ...
        audit.record(actor.userId(), "PERSON_RECORD_PERMANENTLY_DELETED", "ADMINISTRATION",
                ...
                Map.of("operationReference", reference, "organizationId", student.organizationId(),
                        "sourceModule", student.module()), now);
        return new DeletionResult(reference, now);
    }

    private void requireDeletionPermission(String module, Set<String> permissions) {
        String normalized = module == null || module.isBlank() ? "ALL" : module.trim().toUpperCase();
        boolean studentAllowed = permissions != null && permissions.contains("STUDENT_DELETE");
        boolean talentAllowed = permissions != null && permissions.contains("TALENT_DELETE");
        if ("COLLABORATOR".equals(normalized) && !studentAllowed) {
            throw new BusinessException("STUDENT_DELETE_FORBIDDEN", "No tienes permiso para eliminar colaboradores.");
        }
        if ("TALENT_BANK".equals(normalized) && !talentAllowed) {
            throw new BusinessException("TALENT_DELETE_FORBIDDEN", "No tienes permiso para eliminar registros de Talent Bank.");
        }
        if ("ALL".equals(normalized) && !studentAllowed && !talentAllowed) {
            throw new BusinessException("PERMANENT_DELETION_FORBIDDEN", "No tienes permiso para eliminar registros.");
        }
    }
    ...
    public record Candidate(String publicId, String displayName, String email, String status, String module, String talentType,
            String organizationPublicId, String organizationCode, String organizationName, boolean hasCv) {}
