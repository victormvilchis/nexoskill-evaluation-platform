...
public class StudentAuthenticationService {
    ...
    public LoginResult login(LoginCommand command) {
        ...
        auditLogPort.record(null, "STUDENT_LOGIN_SUCCEEDED", "STUDENT_AUTHENTICATION",
                "El estudiante inició sesión.", command.ipAddress(), command.userAgent(),
                Map.of("studentPublicId", student.getPublicId(), "organizationPublicId", organization.getPublicId()), now);
    ...
    public void logout(String rawToken, AuthenticatedStudent principal, String ipAddress, String userAgent) {
        ...
        auditLogPort.record(null, "STUDENT_LOGOUT_SUCCEEDED", "STUDENT_AUTHENTICATION",
                "El estudiante cerró sesión.", ipAddress, userAgent,
                Map.of("studentPublicId", principal.publicId()), now);
    }
    ...
    public AuthenticatedStudent principal(StudentJpaEntity student, OrganizationJpaEntity organization, Instant now) {
        return new AuthenticatedStudent(student.getId(), student.getPublicId(), organization.getId(),
                ...
                student.effectiveStatusAt(now), student.getValidFrom(), student.getExpiresAt(), student.getLastLoginAt(),
                student.isPasswordChangeRequired());
    }
    ...
    private BusinessException invalidLogin(LoginCommand command, StudentJpaEntity student, String reason, Instant now) {
        auditLogPort.record(null, "STUDENT_LOGIN_FAILED", "STUDENT_AUTHENTICATION",
                ...
                Map.of("organizationCode", String.valueOf(command.organizationCode()),
                        "attemptedEmail", String.valueOf(command.email()), "reason", reason,
                        "studentPublicId", student == null ? "" : student.getPublicId()), now);
        return switch (reason) {
            case "TEMP_PASSWORD_EXPIRED" -> new BusinessException("STUDENT_TEMP_PASSWORD_EXPIRED",
