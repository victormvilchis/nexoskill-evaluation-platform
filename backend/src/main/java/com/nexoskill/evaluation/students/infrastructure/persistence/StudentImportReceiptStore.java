...
/**
 * Reserva y cierra confirmaciones de importación en una transacción independiente.
 * El token persistente evita la doble aplicación incluso entre procesos distintos.
 */
@Component
public class StudentImportReceiptStore {
    ...
    public void reserve(String confirmationToken, Long organizationId, String fileSha256,
            int totalRows, Long actorUserId) {
        try {
        ...
        int affected = jdbc.update("""
            UPDATE STUDENT_IMPORT_RECEIPT
             ...
             WHERE CONFIRMATION_TOKEN = :confirmationToken
               AND STATUS_CODE = 'PROCESSING'
            """, new MapSqlParameterSource()
                .addValue("failureCode", truncate(failureCode, 80))
    ...
    public String status(String confirmationToken, Long organizationId, Long actorUserId) {
        List<String> statuses = jdbc.query("""
            ...
            """, Map.of("confirmationToken", confirmationToken,
                ...
                (resultSet, rowNumber) -> resultSet.getString("STATUS_CODE"));
        return statuses.isEmpty() ? null : statuses.getFirst();
    }

    private String truncate(String value, int maxLength) {
        if (value == null || value.isBlank()) return "STUDENT_IMPORT_UNEXPECTED_ERROR";
