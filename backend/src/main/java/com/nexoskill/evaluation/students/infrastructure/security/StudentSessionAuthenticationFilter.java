...
public class StudentSessionAuthenticationFilter extends OncePerRequestFilter {
	private static final Set<String> PUBLIC_PATHS = Set.of("/api/v1/student-auth/login", "/actuator/health", "/error");
	private static final Set<String> PASSWORD_CHANGE_PATHS = Set.of("/api/v1/student-auth/logout",
			"/api/v1/student-auth/change-password", "/api/v1/student/me");
	...
	private boolean authenticate(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (!organization.isOperational(LocalDate.now(clock))) {
			...
			return publicRequest(request)
					|| reject(response, "STUDENT_ACCOUNT_UNAVAILABLE", "La organización no está disponible.");
		}
		...
		if (!student.canAuthenticateOn(today, now)) {
			...
			return publicRequest(request)
					|| reject(response, code, status == StudentEffectiveStatus.EXPIRED ? "Tu acceso ha vencido."
		...
		List<SimpleGrantedAuthority> authorities = List.of(new SimpleGrantedAuthority("ROLE_STUDENT"),
				new SimpleGrantedAuthority("STUDENT_PORTAL"), new SimpleGrantedAuthority("STUDENT_PASSWORD_CHANGE"));
		...
		if (student.isPasswordChangeRequired() && !PASSWORD_CHANGE_PATHS.contains(request.getRequestURI())) {
			return reject(response, "STUDENT_PASSWORD_CHANGE_REQUIRED", "Debes cambiar tu contraseña para continuar.",
