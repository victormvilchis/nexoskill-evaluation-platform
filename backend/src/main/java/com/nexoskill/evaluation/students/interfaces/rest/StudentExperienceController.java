...
public class StudentExperienceController {
	...
	public StudentExperienceService.ExperienceView update(@PathVariable String studentPublicId,
		...
		return service.update(tenantResolver.resolve(request), studentPublicId,
				new StudentExperienceService.UpdateCommand(map(body.currentTechnologies()), map(body.languages()),
						map(body.knownTechnologies())),
				actor.internalId());
	}
	...
	private List<StudentExperienceService.ExperienceItemCommand> map(List<ItemRequest> values) {
		...
		return values.stream()
				.map(value -> new StudentExperienceService.ExperienceItemCommand(value.name(), value.level())).toList();
	}
	...
	public record ItemRequest(@Size(max = 200, message = "El nombre no puede superar 200 caracteres.") String name,
