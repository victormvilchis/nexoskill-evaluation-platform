...
/**
 * Eliminación lógica y transaccional de un colaborador, sin destruir sus
 * históricos.
 */
@Service
public class StudentDeletionService {
	...
	public StudentDeletionService(StudentRepository students, StudentSessionRepository sessions, AuditLogPort audit,
			Clock clock) {
	...
	/**
	 * Conserva el nombre por compatibilidad con clientes anteriores. La operación
	 * es exclusivamente lógica: bloquea la cuenta, revoca sesiones y mantiene
	 * certificaciones, resultados e históricos.
	 */
	@Transactional
	public DeletionResult deletePermanently(TenantContext tenant, String publicId, boolean confirmed,
		...
		audit.record(actor.userId(), "STUDENT_DELETED", "STUDENTS",
				"Se eliminó lógicamente un colaborador y se conservaron sus históricos.", actor.ipAddress(),
				actor.userAgent(),
				Map.of("operationReference", operationReference, "organizationId", student.getOrganizationId(),
						"studentPublicId", student.getPublicId(), "historicalDataPreserved", true),
				now);
