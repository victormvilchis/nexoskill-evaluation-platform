...
public class StudentAuthenticationController {
	...
	public ResponseEntity<LoginResponse> login(@Valid @RequestBody LoginRequest body, HttpServletRequest request,
		...
		StudentAuthenticationService.LoginResult result = service
				.login(new StudentAuthenticationService.LoginCommand(body.organizationCode(), body.email(),
						body.password(), ClientRequestInfo.ipAddress(request), ClientRequestInfo.userAgent(request)));
	...
	public record LoginRequest(@NotBlank @Size(max = 80) String organizationCode,
