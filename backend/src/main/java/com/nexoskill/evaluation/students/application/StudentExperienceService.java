...
public class StudentExperienceService {
	...
	@Transactional
	public void replaceImported(Long organizationId, Long studentId, List<ImportedItem> currentTechnologies,
			List<ImportedItem> languages, List<ImportedItem> knownTechnologies, Long actorId) {
	...
	private void replace(StudentRef student, String type, List<ExperienceItemCommand> values, Long actorId) {
		...
		jdbc.update(
				"DELETE FROM STUDENT_EXPERIENCE_ITEM WHERE STUDENT_ID = :studentId AND ORGANIZATION_ID = :organizationId AND ITEM_TYPE = :type",
		...
		for (ExperienceItemCommand item : normalized) {
			jdbc.update("""
					...
					new MapSqlParameterSource().addValue("publicId", UUID.randomUUID().toString())
							.addValue("studentId", student.id()).addValue("organizationId", student.organizationId())
							.addValue("type", type).addValue("name", item.name())
							.addValue("normalizedName", normalizeKey(item.name())).addValue("level", item.level())
							.addValue("displayOrder", order++).addValue("actorId", actorId));
		}
	}
	...
	private List<ExperienceItemCommand> normalizeItems(List<ExperienceItemCommand> values) {
		...
		for (ExperienceItemCommand item : values) {
			...
			String level = item.level() == null || item.level().isBlank() ? null
					: item.level().trim().toUpperCase(Locale.ROOT);
	...
	private StudentRef requireStudent(TenantContext tenant, String publicId) {
		...
		List<StudentRef> rows = jdbc.query(sql, params,
				(rs, rowNum) -> new StudentRef(rs.getLong("STUDENT_ID"), rs.getLong("ORGANIZATION_ID")));
		if (rows.isEmpty())
			throw new BusinessException("STUDENT_NOT_FOUND", "El colaborador no existe.");
	...
	public static String normalizeKey(String value) {
		if (value == null)
			return "";
		return Normalizer.normalize(value, Normalizer.Form.NFD).replaceAll("\\p{M}+", "").toUpperCase(Locale.ROOT)
	...
	public record ExperienceView(List<ExperienceItem> currentTechnologies, List<ExperienceItem> languages,
			List<ExperienceItem> knownTechnologies) {
	...
	public record UpdateCommand(List<ExperienceItemCommand> currentTechnologies, List<ExperienceItemCommand> languages,
			List<ExperienceItemCommand> knownTechnologies) {
	}
}
