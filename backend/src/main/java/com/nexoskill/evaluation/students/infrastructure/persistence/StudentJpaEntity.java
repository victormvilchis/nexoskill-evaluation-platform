...
public class StudentJpaEntity {
	...
	/**
	 * Columnas legadas conservadas para compatibilidad con instalaciones
	 * anteriores.
	 */
	...
	public static StudentJpaEntity create(String publicId, Long organizationId, String studentCode, String email,
		...
		return create(publicId, organizationId, studentCode, email, normalizedEmail, passwordHash, firstName, lastName,
				displayName, status, validFrom, expiresAt, validFrom, null, null, temporaryPasswordExpiresAt, actorId,
				now);
	}
	...
	public static StudentJpaEntity create(String publicId, Long organizationId, String studentCode, String email,
			...
			String corporateUser, String normalizedCorporateUser, Instant temporaryPasswordExpiresAt, Long actorId,
			Instant now) {
	...
	public StudentEffectiveStatus effectiveStatusOn(LocalDate today) {
		...
		if (recordModule == StudentRecordModule.TALENT_BANK)
	...
	public boolean canAuthenticateOn(LocalDate today, Instant now) {
		return recordModule == StudentRecordModule.COLLABORATOR && status == StudentStatus.ACTIVE
				&& admissionDate != null && (lockedUntil == null || !lockedUntil.isAfter(now));
	}
	...
	public void updateProfile(String studentCode, String email, String normalizedEmail, String firstName,
			String lastName, String displayName, LocalDate validFrom, LocalDate expiresAt, LocalDate admissionDate,
	...
	public void moveToTalentBank(TalentType type, Long actorId, Instant now) {
		if (type == null)
			throw new IllegalArgumentException("El tipo de talento es obligatorio.");
	...
	public void updateTalentMetadata(LocalDate organizationHiredOn, TalentProfileCode profileCode, Long technologyId,
			Long actorId, Instant now) {
	...
	public void softDelete(Long actorId, Instant now, String reason) {
		...
		this.deletionReason = reason == null || reason.isBlank() ? "ELIMINACIÓN LÓGICA" : reason.trim();
		touch(actorId, now);
	}
	...
	public StudentRecordModule getRecordModule() {
		return recordModule;
	}
	...
	public LocalDate getOrganizationHiredOn() {
		return organizationHiredOn;
	}

	public TalentProfileCode getTalentProfileCode() {
		return talentProfileCode;
	}
	...
	public Instant getTalentMovedAt() {
		return talentMovedAt;
	}

	public Long getTalentMovedBy() {
		return talentMovedBy;
	}
	...
	public String getNormalizedCorporateUser() {
		return normalizedCorporateUser;
	}
