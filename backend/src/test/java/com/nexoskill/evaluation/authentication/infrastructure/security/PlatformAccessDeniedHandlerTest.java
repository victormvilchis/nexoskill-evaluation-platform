...
class PlatformAccessDeniedHandlerTest {
    ...
    void returnsSpecificReadOnlyMessageWhenManagerAttemptsCatalogWrite() {
        ...
        verify(audit).record(eq(10L), eq("AUTHORIZATION_DENIED"), eq("SECURITY"),
                eq("Se rechazó una operación sin permisos efectivos."), eq("127.0.0.1"),
                eq("JUnit"), org.mockito.ArgumentMatchers.anyMap(), eq(clock.instant()));
    }
    ...
    void returnsCertificationMessageWhenGlobalAdministratorAttemptsOperationalManagement() {
        ...
        PlatformAccessDeniedHandler.Denial denial = handler.resolveAndAudit(
                request("PUT", "/api/v1/admin/students/student-id/certifications"));
    ...
    private AuthenticatedUser user(String role) {
        return new AuthenticatedUser(10L, "user-public-id", "user@nexoskill.test", "User", "Test",
                "User Test", Set.of(role), Set.of(), null, UserAccessStatus.ACTIVE,
