...
class UserAccountPasswordSecurityTest {
	...
	void shouldCapSessionAtTemporaryPasswordExpiration() {
		...
		assertThat(user.capSessionExpirationForPassword(NOW.plusSeconds(3600))).isEqualTo(NOW.plusSeconds(120));
	}
	...
	private UserAccount user(boolean passwordChangeRequired, Instant temporaryPasswordExpiresAt) {
		return new UserAccount(1L, "public-id", "usuario@nexoskill.local", "USUARIO@NEXOSKILL.LOCAL", "old-hash",
				"Usuario", "NexoSkill", "Usuario NexoSkill", UserStatus.ACTIVE, 0, null, null, passwordChangeRequired,
				null, temporaryPasswordExpiresAt, Set.of(new RoleGrant("USER", Set.of("PASSWORD_CHANGE"))),
				new UserAccess(NOW.minusSeconds(60), NOW.plusSeconds(7200), UserAccessStatus.ACTIVE));
	}
}
