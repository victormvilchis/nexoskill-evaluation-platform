...
class StudentFoundationServiceTest {
	...
	void globalAdministratorMustSelectOrganizationBeforeLoadingCatalogs() {
		TenantContext global = TenantContext.global(1L, "00000000-0000-0000-0000-000000000001", "GLOBAL");
	...
	void rejectsCertificationDataWhenOrganizationDoesNotApplyCertifications() {
		when(organization.isAppliesCertifications()).thenReturn(false);
		StudentFoundationService.CreateCommand command = new StudentFoundationService.CreateCommand(null,
				"student@example.com", "Nombre", "Apellidos", null, StudentStatus.ACTIVE, TODAY, TODAY.plusDays(30),
				TODAY, "00000000-0000-0000-0000-000000000031", null, false, false, false, false, false, false);
		BusinessException exception = assertThrows(BusinessException.class,
				() -> service.create(tenant, command, new StudentService.Actor(7L, "127.0.0.1", "test")));
	...
	void managerCannotSendOrganizationEvenWhenItMatchesTheSession() {
		StudentFoundationService.CreateCommand command = new StudentFoundationService.CreateCommand(
				tenant.organizationPublicId(), "student@example.com", "Nombre", "Apellidos", null, StudentStatus.ACTIVE,
				TODAY, TODAY.plusDays(30), null, null, null, false, false, false, false, false, false);
		BusinessException exception = assertThrows(BusinessException.class,
				() -> service.create(tenant, command, new StudentService.Actor(7L, "127.0.0.1", "test")));
		assertThat(exception.getCode()).isEqualTo("STUDENT_ORGANIZATION_FORBIDDEN");
