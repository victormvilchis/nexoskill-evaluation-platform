...
class ContentReplicationServiceTest {
	...
	void repeatedReplicationReturnsTheExistingCopyWithoutCreatingADuplicate() {
		...
		ContentReplicationLinkJpaEntity existing = ContentReplicationLinkJpaEntity.create(
				"00000000-0000-0000-0000-000000000501", 21L, GlobalContentType.QUESTION, 100L,
				"00000000-0000-0000-0000-000000000100", 3L, 300L, "00000000-0000-0000-0000-000000000300", 7L, NOW);
		when(links.findByOrganizationIdAndContentTypeAndSourceGlobalContentIdAndSourceGlobalVersion(21L,
				GlobalContentType.QUESTION, 100L, 3L)).thenReturn(Optional.of(existing));
	...
	void createsAnOrganizationalCopyAndItsTraceabilityLink() {
		...
		ContentResource global = resource(100L, "00000000-0000-0000-0000-000000000100", ContentScope.GLOBAL, 1L);
		ContentResource target = resource(300L, "00000000-0000-0000-0000-000000000300", ContentScope.ORGANIZATION, 21L);
		when(links.findByOrganizationIdAndContentTypeAndSourceGlobalContentIdAndSourceGlobalVersion(21L,
				GlobalContentType.QUESTION, 100L, 3L)).thenReturn(Optional.empty());
		...
		when(resources.copyToOrganization(GlobalContentType.QUESTION, 100L, 21L, 3L, Map.of(), 7L, true))
				.thenReturn(target);
	...
	private static ContentResource resource(Long id, String publicId, ContentScope scope, Long organizationId) {
		return new ContentResource(GlobalContentType.QUESTION, id, publicId, "Pregunta", "Descripción", "ACTIVE", scope,
				organizationId, null, null, 5L, NOW, 5L, NOW, 3L, false, false, "hash");
	}
}
