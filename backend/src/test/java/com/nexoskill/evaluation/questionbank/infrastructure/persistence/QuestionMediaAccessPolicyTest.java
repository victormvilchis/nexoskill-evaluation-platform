...
class QuestionMediaAccessPolicyTest {
    ...
    void allowsMediaOwnedByCurrentOrganization() {
        ...
        policy.assertAccessible(media, 8L,
                TenantContext.organization(20L, "org-20", "ORG20", false));
    ...
    void hidesMediaOwnedByAnotherOrganization() {
        ...
        assertThatThrownBy(() -> policy.assertAccessible(media, 8L,
                TenantContext.organization(30L, "org-30", "ORG30", false)))
                .isInstanceOfSatisfying(BusinessException.class,
                        exception -> org.assertj.core.api.Assertions.assertThat(exception.getCode())
                                .isEqualTo("QUESTION_MEDIA_NOT_FOUND"));
    }
    ...
    void allowsGlobalMediaOnlyWhenUsedByAQuestionReadableInTheTenant() {
        ...
        when(contentAccessPolicy.canRead(GlobalContentType.QUESTION, 101L,
                ContentScope.GLOBAL, 1L, tenant)).thenReturn(true);

        policy.assertAccessible(media, 8L, tenant);

        verify(contentAccessPolicy).canRead(GlobalContentType.QUESTION, 101L,
                ContentScope.GLOBAL, 1L, tenant);
    }
    ...
    void globalAdministratorMayReadMediaOnlyFromGlobalContext() {
        ...
        policy.assertAccessible(media, 1L,
                TenantContext.global(1L, "global", "GLOBAL"));
    ...
    void preventsOrganizationalMediaFromBeingAttachedToGlobalContent() {
        ...
        assertThatThrownBy(() -> policy.assertAssignable(media, 1L, global, ContentScope.GLOBAL, 1L))
                .isInstanceOfSatisfying(BusinessException.class,
                        exception -> org.assertj.core.api.Assertions.assertThat(exception.getCode())
                                .isEqualTo("QUESTION_MEDIA_SCOPE_MISMATCH"));
    }
    ...
    void rejectsMissingAuthenticatedContext() {
        ...
        assertThatThrownBy(() -> policy.assertAccessible(media, null,
                TenantContext.organization(20L, "org-20", "ORG20", false)))
                .isInstanceOf(BusinessException.class);
    }

    private QuestionMediaJpaEntity media(ContentScope scope, Long organizationId, Long createdBy) {
        QuestionMediaJpaEntity media = QuestionMediaJpaEntity.create(
                "83c62888-2ca1-4d85-a0f6-84ce632f6768", "questions/media.png", "media.png",
                "image/png", 4L, "checksum", scope, organizationId, createdBy, Instant.EPOCH);
        setId(media, 90L);
        return media;
    }
