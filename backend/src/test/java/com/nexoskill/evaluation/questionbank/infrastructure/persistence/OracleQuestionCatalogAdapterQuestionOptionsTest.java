...
class OracleQuestionCatalogAdapterQuestionOptionsTest {
	...
	@Mock
	private SpringDataQuestionTechnologyRepository technologies;
	@Mock
	private SpringDataQuestionCategoryRepository categories;
	@Mock
	private SpringDataQuestionRepository questions;
	@Mock
	private QuestionCategoryStatusHistoryRepository history;
	@Mock
	private OrganizationRepository organizations;
	@Mock
	private GlobalContentAccessPolicy accessPolicy;
	@Mock
	private QuestionCreationTargetResolver creationTargetResolver;
	@Mock
	private ContentSynchronizationService synchronization;
	@Mock
	...
	@BeforeEach
	void setUp() {
		adapter = new OracleQuestionCatalogAdapter(types, difficulties, technologies, categories, questions, history,
				organizations, accessPolicy, creationTargetResolver, synchronization,
	...
	void globalAdministratorSeesGlobalAndSelectedOrganizationCategories() {
		...
		when(categories.findAllByContentScopeOrderByNameAsc(ContentScope.GLOBAL)).thenReturn(List.of(category));
		when(categories.findAllByContentScopeAndOwnerOrganizationIdOrderByNameAsc(ContentScope.ORGANIZATION, 2L))
				.thenReturn(List.of(organizationCategory));
	...
	void globalAdministratorSeesGlobalAndSelectedOrganizationTechnologies() {
		TenantContext tenant = TenantContext.global(1L, "global", "GLOBAL");
		QuestionTechnologyJpaEntity globalTechnology = technology(21L, "00000000-0000-0000-0000-000000000021", "JAVA",
				"Java", 1);
		QuestionTechnologyJpaEntity organizationTechnology = technology(22L, "00000000-0000-0000-0000-000000000022",
				"APX", "APX", 2);
		when(creationTargetResolver.resolve(tenant, "ORGANIZATION", "customer"))
				.thenReturn(new QuestionCreationTargetResolver.Target(ContentScope.ORGANIZATION, 2L, "customer"));
	...
	private static QuestionTechnologyJpaEntity technology(Long id, String publicId, String code, String name,
			int displayOrder) {
