...
class QuestionSaveServiceTest {
    ...
    @Test
    void globalCreationPersistsSelectedAvailabilityWithoutCreatingCopies() {
        ...
        verify(availability).update(eq(question.publicId()),
                eq(new QuestionAvailabilityService.UpdateCommand(
                        QuestionAvailabilityMode.SELECTED_ORGANIZATIONS,
                        List.of("org-20", "org-20", "org-30"))),
                eq(tenant), any(QuestionAvailabilityService.Actor.class));
    }

    @Test
    void globalCreationPersistsGlobalAvailabilityAsReference() {
        QuestionDetail question = question(ContentScope.GLOBAL);
        TenantContext tenant = TenantContext.global(1L, "global", "GLOBAL");
        when(create.execute(isNull())).thenReturn(question);
        ...
        verify(availability).update(eq(question.publicId()),
                eq(new QuestionAvailabilityService.UpdateCommand(QuestionAvailabilityMode.GLOBAL, List.of())),
                eq(tenant), any(QuestionAvailabilityService.Actor.class));
    }

    @Test
    void globalCreationCanRemainUnavailableToOrganizations() {
        QuestionDetail question = question(ContentScope.GLOBAL);
        TenantContext tenant = TenantContext.global(1L, "global", "GLOBAL");
        when(create.execute(isNull())).thenReturn(question);

        service.create(null, QuestionAvailabilityMode.NONE, List.of(), tenant, actor);

        verify(availability).update(eq(question.publicId()),
                eq(new QuestionAvailabilityService.UpdateCommand(QuestionAvailabilityMode.NONE, List.of())),
                eq(tenant), any(QuestionAvailabilityService.Actor.class));
    }
