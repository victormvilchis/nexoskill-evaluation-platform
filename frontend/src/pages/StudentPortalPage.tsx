...
export function StudentPortalPage() {
  ...
  const activePath = home.paths.find((item) => item.status === 'IN_PROGRESS') ?? home.paths.find((item) => item.status === 'ASSIGNED')
  const completedPaths = home.paths.filter((item) => item.status === 'COMPLETED').length
  healthDimensions.push({ label: 'Desarrollo', value: home.paths.length === 0 ? 'Sin Path asignado' : activePath ? `${activePath.name} ${activePath.status === 'IN_PROGRESS' ? 'en progreso' : 'asignado'}` : completedPaths === home.paths.length ? 'Paths completados' : `${home.paths.length} Paths activos`, tone: activePath?.status === 'IN_PROGRESS' || completedPaths > 0 ? 'good' : 'neutral' })
  return <div className="development-home">
    ...
    <section className="development-section student-home-paths"><div className="development-section-heading"><div><p className="development-eyebrow">Ruta de desarrollo</p><h2>Mis Paths</h2></div><button className="development-link-button" onClick={() => navigate('/student/paths')}>Ver todos</button></div>{home.paths.length===0?<div className="development-empty-inline student-path-empty-inline"><strong>Actualmente no tienes Paths asignados.</strong><span>Puedes continuar con tus demás actividades de aprendizaje.</span></div>:<div className="student-home-path-grid">{home.paths.slice(0,3).map((path)=><article key={path.assignmentPublicId} className="student-home-path-card"><div><span className={`development-badge ${path.status.toLowerCase()}`}>{path.status==='COMPLETED'?'Completado':path.status==='IN_PROGRESS'?'En progreso':path.status==='EMPTY'?'Sin contenido evaluable':'Asignado'}</span><h3>{path.name}</h3><p>{path.completedStages} de {path.totalStages} etapas completadas</p>{path.nextStageTitle&&<small>Siguiente: {path.nextStageTitle}</small>}</div><button className="development-link-button" onClick={()=>navigate(`/student/paths/${path.assignmentPublicId}`)}>{path.nextEvaluationAssignmentPublicId?'Continuar →':'Ver recorrido →'}</button></article>)}</div>}</section>
