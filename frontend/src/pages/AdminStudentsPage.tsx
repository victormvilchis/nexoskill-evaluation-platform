...
import type { StudentCertificationFocusStatus, StudentFilterOptions, StudentPage } from '../shared/types/students'
import { formatPersonName } from '../shared/utils/personNames'

const certificationFocusOptions: Array<{ value: StudentCertificationFocusStatus | ''; label: string }> = [
  { value: '', label: 'Todos' },
  { value: 'IN_RULE', label: 'En regla' },
  { value: 'EXPIRING_SOON', label: 'Próxima a vencer' },
  { value: 'EXPIRED', label: 'Vencida' },
  { value: 'PENDING_DEACTIVATION', label: 'Pendiente de Baja' }
]

const certificationFocusLabels: Record<StudentCertificationFocusStatus, string> = {
  IN_RULE: 'En regla',
...
export function AdminStudentsPage() {
  ...
  const [filterOptions, setFilterOptions] = useState<StudentFilterOptions>({
    organizations: [], roles: [], technologies: [], certificationStatusAvailable: true
  })
  ...
  const organization = administrator ? (searchParams.get('organization') ?? '') : ''
  const role = searchParams.get('role') ?? ''
  const technology = searchParams.get('technology') ?? ''
  const certificationFocus = searchParams.get('certificationFocus') ?? ''
  const debouncedQuery = useDebouncedValue(query, 300)

  useEffect(() => {
    const controller = new AbortController()
    getStudentFilterOptions(organization || undefined, controller.signal)
  ...
  useEffect(() => {
    ...
    if (currentQuery === debouncedQuery.trim()) return
  ...
  useEffect(() => {
    ...
    searchStudents({
      ...
      if (!controller.signal.aborted) {
        setError(requestError instanceof ApiRequestError ? requestError.message : 'No fue posible consultar colaboradores.')
      }
    }).finally(() => { if (!controller.signal.aborted) setLoading(false) })
    return () => controller.abort()
  }, [administrator, certificationFocus, organization, page, role, searchParams, setSearchParams, size, technology])
  ...
  function updateParam(name: string, value: string) {
    ...
    if (name === 'organization') {
      ...
      next.delete('certificationFocus')
    }
    setSearchParams(next)
  }

  function toggleSort(nextSort: 'displayName' | 'admissionDate' | 'certificationStatus') {
    const currentSort = searchParams.get('sort')
    const currentDirection = searchParams.get('direction') === 'DESC' ? 'DESC' : 'ASC'
    const defaultDirection = nextSort === 'certificationStatus' ? 'DESC' : 'ASC'
    const nextDirection = currentSort === nextSort
  ...
  const showCertificationColumns = filterOptions.certificationStatusAvailable
  const columnCount = showCertificationColumns ? 7 : 5
  ...
  return (
    <main className="content-page resource-page ns-list-page student-page student-global-page">
      ...
      <FilterToolbar hasActiveFilters={Boolean(query || organization || role || technology || certificationFocus)} onClear={clearFilters}>
        {administrator && <ResourceSelectField label="Organización" value={organization} onChange={(value) => updateParam('organization', value)} width="wide">
        ...
        {showCertificationColumns && <ResourceSelectField label="Status Certificación" value={certificationFocus} onChange={(value) => updateParam('certificationFocus', value)} width="medium">
          {certificationFocusOptions.map((option) => <option key={option.value || 'ALL'} value={option.value}>{option.label}</option>)}
        </ResourceSelectField>}
      </FilterToolbar>
          ...
          <table className="ns-data-table">
            <thead><tr>
              <th aria-sort={activeSort === 'displayName' ? (direction === 'ASC' ? 'ascending' : 'descending') : 'none'}>
                <button className={`ns-sortable-column-button${activeSort === 'displayName' ? ' active' : ''}`} type="button" onClick={() => toggleSort('displayName')}>Colaborador <SortIndicator active={activeSort === 'displayName'} direction={direction} /></button>
              </th>
              <th>Rol</th>
              <th>Tecnología actual</th>
              <th aria-sort={activeSort === 'admissionDate' ? (direction === 'ASC' ? 'ascending' : 'descending') : 'none'}>
                <button className={`ns-sortable-column-button${activeSort === 'admissionDate' ? ' active' : ''}`} type="button" onClick={() => toggleSort('admissionDate')}>Fecha de alta <SortIndicator active={activeSort === 'admissionDate'} direction={direction} /></button>
              </th>
              {showCertificationColumns && <th>Vencimiento</th>}
              {showCertificationColumns && <th aria-sort={activeSort === 'certificationStatus' ? (direction === 'ASC' ? 'ascending' : 'descending') : 'none'}>
                <button className={`ns-sortable-column-button${activeSort === 'certificationStatus' ? ' active' : ''}`} type="button" onClick={() => toggleSort('certificationStatus')}>Status Certificación <SortIndicator active={activeSort === 'certificationStatus'} direction={direction} /></button>
              </th>}
            ...
            <tbody>
              ...
              {!loading && data?.content.map((student) => <tr key={student.publicId}>
                <td className="ns-primary-cell"><span className="ns-person-name">{formatPersonName(student.displayName)}</span><small>{student.email?.trim() || 'N/A'}</small></td>
                ...
                {showCertificationColumns && <td>{formatDate(student.certificationFocusExpirationDate)}</td>}
                {showCertificationColumns && <td>{student.certificationFocusStatus
                  ? <span className={`status-badge certification-focus certification-focus-${student.certificationFocusStatus.toLowerCase().replaceAll('_', '-')}`}>{certificationFocusLabels[student.certificationFocusStatus]}</span>
                  : <span className="muted">No aplica</span>}</td>}
                <td className="ns-actions-column"><TableActions>
                  ...
                  {student.organization?.appliesCertifications && (administrator || permissions.has('STUDENT_CERTIFICATION_VIEW') || permissions.has('STUDENT_CERTIFICATION_MANAGE')) && <TableActionLink icon="clipboard" label="Certificaciones" to={`/admin/collaborators/${student.publicId}/certifications`} />}
