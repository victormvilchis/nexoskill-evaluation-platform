...
import type { QuestionPage } from '../shared/types/questions'
    ...
    <main className="content-page resource-page">
      <div className="page-heading resource-heading">
        <div>
          ...
          <p className="muted">
            Preguntas reutilizables organizadas por categorías y colecciones.
          </p>
        </div>
      ...
      {error && (
        <section className="inline-error-panel" role="alert">
          ...
          <div>
            <strong>No fue posible cargar el banco de preguntas</strong>
            <p>{error}</p>
          </div>
      ...
      {loading && !data ? (
        <div className="resource-card-grid" aria-label="Cargando preguntas">
        ...
        <div className="resource-card-grid">
          {questions.map((item) => (
            <Link
              className="resource-card question-resource-card"
              to={`/admin/questions/${item.publicId}`}
              key={item.publicId}
            >
              ...
              <footer>
                ...
                {item.hasMedia && (
                  <span>
                    <Icon name="image" size={14} /> Multimedia
                  </span>
                )}
              </footer>
            </Link>
      ...
      {!loading && !error && questions.length === 0 && (
        <section className="empty-state-card resource-empty-state">
          ...
          <h2>{query ? 'No encontramos coincidencias' : 'Aún no hay preguntas'}</h2>
          <p>
            {query
              ? 'Prueba con otro término de búsqueda.'
              : 'Crea la primera pregunta para comenzar a construir tu banco.'}
          </p>
          {!query && (
