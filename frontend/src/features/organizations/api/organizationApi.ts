...
import type { OrganizationDetail, OrganizationPage, OrganizationPayload, OrganizationStatus } from '../types/organizations'
...
export function getOrganization(publicId: string) {
...
export function changeOrganizationStatus(publicId: string, status: OrganizationStatus) {
  return apiRequest<OrganizationDetail>(`/admin/organizations/${publicId}/status/${status}`, { method: 'POST' })
}
