...
export function TalentCvUploadField({ file, disabled = false, error, onChange }: TalentCvUploadFieldProps) {
  const inputId = useId()
  ...
  return (
    <div className="talent-cv-upload-field">
      ...
      <div className={`talent-cv-dropzone${dragging ? ' is-dragging' : ''}${disabled ? ' is-disabled' : ''}`}
        ...
        <div><strong>{file ? file.name : 'Seleccionar currículum vitae'}</strong><span>{file ? 'El archivo está listo para guardarse.' : 'Arrastra el archivo aquí o selecciónalo desde tu equipo.'}</span></div>
        <span className="secondary-button talent-cv-select-button">{file ? 'Cambiar archivo' : 'Seleccionar archivo'}</span>
      </div>
